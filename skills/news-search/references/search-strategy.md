...
### Stop when:
- You have searched all news-search dimensions (D1-D8 plus D10) for a given paper/tool, treating D10 as a corpus-level pass that may have already been run for the round, and confirmed the citation-audit hook freshness state (see `skills/citation-audit/SKILL.md`; D9 is now its own skill)
...
Tag each entry with the dimension that found it (D1-D8 plus D10; citation-affiliation evidence comes from the standalone citation-audit skill, not from a news-search dimension) so future audits know which strategies produced results.

...
- **OpenAlex** (no credentials required). Standard open citation graph. Undercounts CS papers severely -- PyOD shows ~24 citations on OpenAlex versus 1,000+ on Google Scholar -- but covers a broad swath of the literature and is the safe no-credential default. For recent arXiv-only work the undercount is often a hard zero: in the 2026-07-19 news-search round OpenAlex reported **0** citations for both Auditable Agents (arXiv:2604.05485) and Implicit Execution Tracing (arXiv:2603.17445), while direct PDF verification found seven and several respectively. Read an OpenAlex zero on a recent CS preprint as "not yet indexed," and verify before recording it.
...
- **OpenAlex false zeros on recent CS preprints.** Beyond undercounting, OpenAlex often reports a flat **0** for recent arXiv-only papers that already have real citations. The 2026-07-19 news-search round confirmed this on two lab papers (Auditable Agents, arXiv:2604.05485, OpenAlex 0 vs. 7 verified; Implicit Execution Tracing, arXiv:2603.17445, OpenAlex 0 vs. several verified). Because `--source openalex` is the default and needs no credentials, a run over a fresh-preprint-heavy inventory will under-report across the board. Mitigation: for any paper under about 12 months old that OpenAlex returns as zero, cross-check with the Semantic Scholar citations API (`api.semanticscholar.org/graph/v1/paper/arXiv:<id>/citations`) or the news-search follow-up before recording a zero, and prefer `--source both` when Dimensions credentials exist. An OpenAlex zero must not silently zero out a paper in the audit tables.

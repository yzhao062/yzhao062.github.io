...
One net-new Tier 0 document, Ledger 1 row 21, which is a NICT presentation submitted to a Japan
MIC committee rather than an independent adoption. The round's most consequential output is not a
promotion. For four rounds the
Summary Statistics table has been wrong, and the per-ledger count lines it copied had stopped tracking
their own tables. That is written up under "The Published Total Was Never Reproducible"
below. The largest verified addition is the patent figure, which moves from 49 to **94** on a basis
that is now stated and checkable.

### What the Round Ran

Phase A dispatched 37 Agy lanes and all 37 returned, with zero FALLBACK records. They produced 1,014
records over 956 unique URLs, of which 779 were absent from the identity index. Phase B verified
every candidate it was given: 16 units over **378 candidates**, no deferrals. Two further units
audited surfaces rather than candidates, one over the 94 patent publication numbers and one over the
9 award links on the site and in the CV.

The 378 verdicts split as follows.

| Verdict | Count |
|---|---|
| `candidate-promote` | 243 |
| `already_counted` (reached by a new URL) | 77 |
| `dropped` | 27 |
| `verified-negative` | 15 |
| `paywall_or_blocked` | 12 |
| `topic-validation` | 4 |
| **Total** | **378** |

Of the 243 promotions, one was refuted on re-fetch and two were the same document reached twice, so
**240 promotions** were placed. They land as Ledger 1 +1, Ledger 2 +14, Ledger 3 +117 items across 83
rows, Ledger 6 +3, Ledger 7 +30, and 75 records handed to `/citation-audit`. Those six figures sum to
240, which is the check that no promotion was placed twice or lost.

### The Published Total Was Never Reproducible

The Summary Statistics table published **582** as "the sum of the eight ledgers above". Those eight
figures sum to 364. No arithmetic connects the two, and none of the rounds that
quoted 582 checked it against the rows it claimed to summarize.

The second fault is larger and explains the first. Each per-ledger count line had stopped tracking its
own table:

| Ledger | Published count | Rows the table actually held |
|---|---|---|
| Ledger 2 | 80 | 108 |
| Ledger 3 | 170 | 331 |
| Ledger 6 | 11 | 34 |
...
Rounds were appending rows and writing the promotion into the pass section without updating the count
line under the table. The Summary then copied the stale count lines rather than counting the tables.
Both figures are now taken from the live tables: **731 rows carrying 771 counted items**. Each count
line carries the same number as the table above it, and the Summary states, per ledger, the rule that
separates rows from items.

The recount is not growth. This round's verification added 165 items, and one more is a 2026-09-12
promotion whose row is written here for the first time. The pre-round tables held 605 items against a
published 582, so 23 items were on the tables and outside the total before this round began.

Two smaller placement faults surfaced in Ledger 1b during the recount. The first is recorded rather
than fixed: the table's basis is DOE national-laboratory reports with substantive use, and rows G9
through G12, appended on 2026-08-13, fall outside it. Two of them (arxiv.org, pubs.rsc.org) are not
government documents at all. They stay uncounted until a round re-places them. The second is fixed.
On 2026-09-12 a pass promoted SLAC OSTI 3005876, MetaOD's first appearance in a government document,
and never wrote its row. It is now row G13, with the OSTI record and DOI re-fetched on 2026-09-22.

### Patents: 49 to 94, With the Basis Stated

The site published 49. Ledger 3 of this audit carried 89 rows with a patent publication number,
which is the figure the 49 should have tracked. This round added 13 more, giving 102 rows.

Eight of those 102 rows are a second or third publication of an invention another row already holds.
Four are CN applications published twice, as A and B kind codes: rows 159/234, 160/237, 164/247 and
165/250. Rows 176 and 269 are the two SK utility-model publications of one filing. Two more are family
members of row 66m's EP4662606A1, the KR row 175 and the CN row 266. The eighth, row 274, is the WO
publication of row 66cl's US20230017157A1. Subtracting those eight leaves **94 distinct inventions**.

All 13 additions were family-checked against the existing rows before being counted, by reading each
one's Google Patents sibling list and intersecting it with the numbers already on file. Zero were
family duplicates. The sibling lists ran from 12 to 36 publications each: CN113761292B (31),
CN115293481A (36), CN115567572B (34), CN116167004A (18), CN116633666A (30), CN117113250B (14),
CN117453502A (25), CN118606861A (27), CN118798628A (12), CN119937461A (25), CN120671044A (15),
CN121814397A (25) and EP4451150A1 (33). Two of the 13 timed out on the first attempt and resolved on
retry, which is the tool-reliability rule doing its job.

One caveat keeps the arithmetic honest. Two rows name more than one number for a single invention.
Row 66cn names US12074893B2 with US11711391B2 and WO2022082091A1 as siblings, and row 346 names
both the A and B publications of CN115293481. That is why the count of distinct publication numbers,
105, is higher than the invention count of 94. The basis published
on every surface is inventions, not publication numbers.

The jurisdiction split over the 94 is China 70, US 14, WIPO 3, EPO 3, Germany 1, Japan 1, Korea 1,
Slovakia 1. Note that the previous split labelled the EPO entries "EU", which is wrong: EP
publications come from the European Patent Office, whose membership is not the European Union.

### The Tier Rule Was Rewritten and Then Reverted the Same Day

`SKILL.md` said a Phase A tier is a ceiling, on evidence of 110 down and zero up over 420 candidates.
This round rewrote it to say a tier is a ceiling on a calibrated surface and a floor on a new one. The
evidence offered was 110 down and 20 up over 378 candidates, with 14 of the 20 from the thesis unit.

Two reviewers challenged the rewrite as too thin to justify on one round over one new surface. A
recount found something worse than thin evidence. The verdict records cannot produce
the claim at all: `tier_was` was written on 70 of 481 records, and only 56 carry both `tier_was` and
a final tier. A movement count over 378 candidates was never available. The rule is reverted, the
reason is recorded next to it, and `tier_was` is now a required field in the verdict schema rather
than an optional one.

This is the round's method result, and it generalizes past this rule. A calibration claim needs the
field that measures it to be mandatory at capture time, because nothing downstream can reconstruct a
tier that was never written down.

### Award Links: Five Replaced, One Unlinked

A dedicated unit fetched all 9 award links on the site and in the CV. Two name the PI and are
unchanged (Meta AI4AI and the Google Cloud research innovators directory). One is dead: Gen Digital
retired the whole Norton Labs news tree. The other six are live and name nobody, which makes them
decoration rather than evidence. Capital One accounts for two of the six, because `index.html` and
the CV linked the same award to two different pages and neither names a recipient. Each replacement
below was fetched and confirmed to name the PI in the award's own context.

| Award | Replacement | What the page says |
|---|---|---|
| AAAI New Faculty Highlights | `aaai.org/aaai-24-conference/nfh-24-program/` | The program lists "Towards Reproducible, Automated, and Scalable Anomaly Detection / Yue Zhao". Before, the link pointed at the call for proposals, which names nobody. |
| ACM SIGSPATIAL | `sigspatial.org/people/award-recipients/` | The recipients index names "Yue Zhao, University of Southern California" for TyphoFormer. Before, the link pointed at the 2025 conference home page. |
| Capital One | `capitalone.com/tech/ai/credif-spring-2025-ai-award-fellowship-recipients/` | Capital One's own recipients announcement. It replaces both previous links: the USC Viterbi article in the CV, which announces the creation of the center, and the CREDIF page in `index.html`. Neither named a recipient. |
| Foresight Institute | `foresight.org/grantees/2026-yue-zhao/` | A grantee page for the PI. The previous link now 301s to a live call for proposals. |
| NortonLifeLock | Wayback capture `20240414184544` of the 2022 fellowship winners page | The live page is dead with no successor anywhere on the Gen Digital site; the capture names the winners. |

The NVIDIA Academic Grant Program link is **unlinked rather than replaced**. The fetch found no
recipient list for the program and no public page naming this award and this recipient together. Pointing at the program description asserted evidence that does not
exist. The award text remains; only the link is gone. Both reviewers reached
this conclusion independently and both were right.

One disagreement is left for the user rather than resolved here. The CV dates the Capital One award
October 2024, while the replacement link is a spring 2025 recipients announcement. Either the date or
the link is wrong, and the audit cannot tell which from the outside.

### Handoffs and What This Round Did Not Do

The 75 records routed to `/citation-audit` are a tracked manifest at
`skills/news-search/handoffs/2026-09-22-citation-audit.jsonl`, one JSON object per record. Each
carries the cited work, the citing source, the resolved URL, the evidence quote, the citation
location, the bibliography format and a self-citation verdict. Importing them needs no new OpenAlex
pass, because every record was already fetched and read. Each self-citation verdict is read from the
verifier's notes and never inferred. Thirty-two are independent, and 3 pass the literal screen with a
proximity caveat. One is flagged as advisor-adjacent, and one is excluded because the PI's frequent
co-authors sit on the citing byline. For the remaining 38 the verifier recorded no screen, and those
need one before any of them is counted anywhere.

One record in that manifest is worth naming. *AI and Ethics* (Springer, 2026, single author Alex
Chengyu Li) cites Auditable Agents, the first peer-reviewed journal citation of the paper found so
far. It is not a Ledger 6 row, because Ledger 6 admits a citation only with body discussion or a
named-mechanism reference. This one is a reference-list entry in an article whose body is paywalled,
so the depth of engagement is unknown. An earlier draft of this round had placed it in Ledger 6.
Each of the three rows there instead, A35 to A37, quotes body text. A35 is a prior-art summary of
Auditable Agents (arXiv:2609.12582). A36 describes MemoHarness's mechanism in an NVIDIA, NTU and MIT
paper (arXiv:2609.20519). A37 is SteerBench-Work calling The Autonomy Tax "the closest prior
observation" to its own finding.

Ledger 3's 117 new items are carried on 83 rows, not 117. Six cluster rows hold 40 items between
them, covering the aggregator, tutorial, community, video, blog and platform classes, following the
folding convention used on 2026-06-02. Each cluster row carries one link per item, so every counted
item keeps its own address in the tracked file. Writing 117 separate narrative rows for
forum comments and marketplace listings would have grown the table by a third to carry the least
consequential evidence in it.

Three patent candidates stay uncounted, as they have since at least 2026-06-14, and the 94 does
not include them. They are CN112989338B (Tencent, COPOD at ¶0133 and ¶0165), CN117216660B
(DBAPPSecurity, TODS paraphrased as prior art at ¶0003) and CN117648656A (Chongqing University,
ADBench at ¶0125). Each needs the same two steps before it can count. Fetch the Google Patents page
and confirm the tool at the cited paragraph from the page text itself. Then intersect its sibling
list with the 105 publication numbers already on file. A row and a count change belong in the same
commit as that check.

Two mismatches between a table's rule and its practice surfaced, and both are left for the user.
Ledger 7 is titled peer-reviewed and its inclusion rule says so, yet it has admitted arXiv preprints
with substantive use since row S16. This round's S70, S77 and S78 follow that practice. Separately,
Ledger 2 rows 50 through 63, written on 2026-09-12, carry five cells under an eight-column header,
so they render misaligned. Neither is fixed here, because each needs a decision about the table
rather than about a row.

Three claims that an earlier draft of this section made are withdrawn, because the round cannot
support them. One is a tier-movement measurement over 378 candidates, for the reason given above.
Another is a count of coverage across five discovery dimensions, which nobody computed from the lane
outputs. The last is a reading of what OpenAlex returned. It rests on the empty outgoing-reference
metadata that the 2026-09-12 pass already recorded as a limit on what a retrieval failure there can
mean.
...
| Surface | Change |
|---|---|
| `opensource.html` | Patent row 49 to 94, with the jurisdiction split and the "distinct inventions" basis. |
| `files/bio.txt` | "cited in 49 patents" to 94. |
| `llms.txt` | Patents bullet to 94 with the corrected split. |
| `index.html` | Bio prerender to 94; NVIDIA award unlinked; four award links replaced. |
| `cv/cv-full.tex`, `cv/cv-1page.tex` | Award links replaced; NVIDIA award unlinked. |
| `scripts/ci_check_site.py` | `index.html` added to the patent-count claim map. It carries the same bio sentence as `files/bio.txt` and was the one surface the gate did not watch. |

---

## 2026-09-12 Pass (general round: 32 Agy lanes via `/prun` + 9 Claude agents via `/workflows`; auditing-line round: 12 Agy lanes + 13 Claude agents; Phase B: 16 Agy lanes over 420 candidates; Phase C: 6 Agy lanes over everything still blocked)
...
confirmed by fetching full text. Phase C added two more by reading their Crossref-deposited
bibliographies, so the count is **nineteen**: 10.3390/ai7080298 (MDPI *AI*, 112 references) and
10.1145/3837083 (ACM Computing Surveys, 195 references) each omit all fourteen, and the ACM survey
cites TrustLLM while doing so. Seven surveys do cite the line, after Phase B verified three more:
...
standing negative and it corrects the earlier zero, and Phase C established the cause: Crew Scaler's
entry is reference **[991]**, beyond the depth cap automated reference parsers apply, and it uses the
title variant "Attribution" rather than the "Auditing" in `data/publications.json`; DEMM is PDF-only on
arXiv with no HTML5 metadata to extract. Two separate parser limits, and a "hard zero across eight
indexes" reveals neither.
...
| The Autonomy Tax has no verified citer | **Falsified.** OmniScientist (arXiv:2608.13558, NUS and Oxford) cites it in its Introduction as foundation for agent tool use, alongside ReAct, Toolformer and Reflexion. Ledger 6 row A34. |
| GRADE has no verified citer | **Was never true, and this document already held the proof.** Ledger 6 row A19 (TraceCompiler, arXiv:2608.02680) and Ledger 3 row 77 ("Doomed from the Start", arXiv:2607.06503) were both recorded before the sweep that reported the zero. Phase C re-fetched both. |
| WeClawArena, SkillCenter, Agent Safety Is Action Alignment, CatchBench and IET have no external academic citer | **Holds**, across arXiv full text, ACL Anthology, OpenReview, thesis repositories and Asian scholarly databases. Read it as a statement about the literature only: three of the five have real code or editorial adoption recorded in Ledger 3 this round. WeClawArena's Anthology absence has a reason rather than a verdict, since EMNLP 2026 Findings is not yet published there. |

All four zeros this pass attributed to the verification cap are now falsified, and one of the four had
never been a zero. Recording them as explicit reversals rather than editing the numbers in place
follows the IET case above. A zero that a later fetch overturns is a finding about the method, and
correcting it silently destroys the only evidence that the method produced it.
...
per-work zeros were therefore cap artifacts rather than measured absences: GRADE, The Autonomy Tax,
No Attacker Needed and MemoHarness all returned candidates that nobody verified. **All four have since
been falsified**, No Attacker Needed and MemoHarness in Phase B and The Autonomy Tax in Phase C, and
GRADE turned out to have had two citations recorded in this document all along. Four named cap
artifacts, four reversals, none of them a measured absence.
...
Publisher full text stayed shut to a direct fetch, and Phase C got both answers anyway through
Crossref's deposited bibliographies: 10.3390/ai7080298 (MDPI) cites nothing of this lab's across 112
references, and 10.1145/3837083 (ACM Computing Surveys) cites TrustLLM at reference 77 across 195.
Neither is a zero, and neither needed the paywall. Of the twelve O'Reilly items, five were already
recorded and six were verified through Packt companion repositories and previews; of the Medium and
Zhihu blocks, four of nine yielded. What genuinely remains behind access control is small and named in
the Phase C section: two Gartner documents, one S&P Global 451 page, one member-only Medium post, three
Zhihu pages, and one book chapter.
...
### Phase C: Route-Around Verification of Everything Still Blocked

Phase B left 23 candidates unreachable and this pass carried five more gated documents, two
publisher-blocked surveys, six repository leads a sweep named but never fetched, and seven per-work
zeros resting on method rather than on a fetch. Every one of those is blocked on a **route** rather
than on a fact, so a third round ran 6 Agy lanes sliced by the alternate route each had to try.
6 of 6 complete, zero FALLBACK, 38 records.

| Outcome | Count |
|---|---|
| CONFIRMED | 22 |
| still blocked, every route named | 8 |
| verified-negative | 6 |
| dropped | 2 |

**Most blocks are bot detection rather than access control.** 25 blocked or gated items entered these
lanes and **17 reached a decision**; 8 stayed blocked with every route named. Three more were settled
locally before dispatch by the identity check described below, because they were already in this record.
The routes that worked are worth naming because none requires a subscription:

| Route | What it settled |
|---|---|
| **Crossref REST API** (`api.crossref.org/works/<doi>`) | Publishers deposit their **full reference lists** to Crossref for free. This settled both "must not be recorded as zeros" surveys in two requests, through a 403 at both publishers. |
| **Packt companion repositories** on GitHub | 5 of 9 subscription-reader books. A `from pyod.models.knn import KNN` in the chapter's own notebook is stronger evidence of use than the prose would have been. |
| **Stack Exchange API v2.3** | The Stack Overflow thread, through a Cloudflare challenge on the web page. |
| **Browser User-Agent on Medium** | 3 of 4 Medium posts. Medium's 403 is bot detection; only one of the four is genuinely member-only. |
| **`raw.githubusercontent.com`** | All 6 repository leads. |
| **Google Books preview, publisher TOC, public translation mirror** | 3 books with no companion repository. |

No paywall was bypassed. Where a claim sits behind one, the record says `still blocked` and names the
routes tried: two Gartner documents, one S&P Global 451 Research product page, one member-only Medium
post, three Zhihu pages behind an anti-bot captcha that survived four routes, and one Packt book whose
chapter topic is confirmed while the TrustLLM citation inside it is not.

**The Crossref reference-list route is the find worth carrying into the next round.** The audit's
standing note that publisher-side full text is "the only route to 131 journal-only citing works" is
wrong in one direction: for any work with a Crossref DOI, the deposited bibliography answers "does this
paper cite X" without reading the paper. It does not answer *where* in the body, so it establishes a
citation and not a Ledger 7 admission. Both blocked surveys resolved this way:

- **10.3390/ai7080298** is "Agentic AI Safety: A Structured Review of Open Problems and Their
  Regulatory Anchoring" (MDPI *AI*, 2026-08-04; Valenta, Rozinek, Horálek). **112 deposited references
  and not one names any work of this lab's.** A verified negative, and the eighteenth survey in this
  pass's survey census that omits the whole line.
- **10.1145/3837083** is "A Comparative Survey of Security Risks in AI Systems: From LLMs to AI Agents
  and Embodied Agents" (**ACM Computing Surveys**, 2026-08-22; Zhejiang University and Stony Brook).
  195 deposited references, and **reference 77 is TrustLLM**. A new citation at the field's highest-impact
  survey venue. Body placement is unknown behind the ACM wall, so this routes to `/citation-audit`
  rather than to Ledger 7. It also omits all fourteen auditing-line works, making it the nineteenth.

**Six leads a sweep found and never fetched: six confirmations, no bot digests, no first-party.** These
were recorded as "thin but non-zero" and written off. Fetched, they are: `Stage-11-Agentics/etch` citing
Implicit Execution Tracing by full title and arXiv ID, which is that work's only code-search footprint;
`choiceoh/Deneb`, which **implements** SkillCenter's source-grounding rule in Go rather than citing it;
`endomorphosis/ipfs_datasets_py` (22 stars) naming SkillCenter only through repository and Hugging Face
dataset handles, so no identifier-keyed search can see it; `ydyjya/Awesome-LLM-Safety` at **1,909 stars**
carrying Agent Safety Is Action Alignment with title, arXiv link and USC affiliation; `Tibsfox/gsd-skill-creator`
(69 stars) encoding the same paper's argument and quoting its numbers; and `smirby/aiextinctionrisk`
analysing it in an independent blog post.

Taken with the six deferred priority candidates that all verified earlier in this pass, **every lead
this audit deferred for lack of budget has since verified: twelve for twelve, across two batches
triaged by different methods.** That is now a measured property of the triage rather than an anecdote,
and it is the strongest argument this record contains against reporting a cap as if it cost nothing.

**Two standing zeros fall, and one was never a zero.**

- **The Autonomy Tax is cited.** OmniScientist (arXiv:2608.13558; Bobo Li, Hao Fei, Tianjie Ju, Mong Li
  Lee, Wynne Hsu, NUS and Oxford) cites it in its Introduction as foundation for agent tool use,
  alongside ReAct, Toolformer and Reflexion. This was the last of the four cap-artifact zeros still
  standing; all four are now falsified.
- **GRADE was never at zero, and this record already held the proof.** Two sweeps reported GRADE as
  having no external citer while **Ledger 6 row A19 (TraceCompiler, arXiv:2608.02680) and Ledger 3 row
  77 ("Doomed from the Start", arXiv:2607.06503) were already in this document**. Phase C re-confirmed
  both by fetching them. This is the round's dedup defect running in the opposite direction from every
  other instance: the lanes were given a suppression list so they would not re-report known coverage,
  and nobody ran the *zeros* question against the ledger, so a work with two recorded citations was
  published here as a zero. Check a zero against the record before reporting it, exactly as a candidate
  is checked.

**Five zeros hold, and the distinction between two kinds of zero now matters.** WeClawArena,
SkillCenter, Agent Safety Is Action Alignment, CatchBench, and Implicit Execution Tracing have **no
external academic citer**, confirmed across arXiv full text, ACL Anthology, OpenReview, theses
repositories and Asian scholarly databases. Three of those five have real non-academic adoption, found
in the same round: SkillCenter has two code adoptions, Agent Safety has three list-and-blog items, and
IET has one repository citation. A work can be adopted in code and uncited in the literature at the
same time, and a row that reports one number hides the other. WeClawArena's ACL Anthology check also
came back with a reason rather than a zero: **EMNLP 2026 Findings is not yet published in the
Anthology**, so its record cannot exist there yet.

**The IET index blindness finally has a mechanism.** This pass recorded IET's falsified zero with the
note that "whose precise cause remains unverified". Phase C names it: Crew Scaler's citation is
reference **[991]**, past the depth cap automated parsers apply to reference lists, and it uses the
title variant "Attribution" where this repo's own metadata says "Auditing"; DEMM is PDF-only on arXiv
with no HTML5 metadata to extract. Two different parser limits, neither of which a "hard zero across
eight indexes" reveals.

**Two more locators were mangled rather than invented**, which is the milder form of the Phase B
failure: the Towards Data Science COPOD URL carried a three-character suffix typo, and the Medium slug
was truncated mid-title. Both resolved to real articles once the canonical URL was recovered. Phase B's
five invented addresses and these two corrupted ones are the same failure at different severities, and
both are caught the same way: resolve the locator before believing the row.

**The already-counted rate on blocked items is as high as on any other class.** Of the 12 O'Reilly
items, five were already in this record: two books by title at rows 37 and 39, one video edition of row
37, and two Packt titles at rows 195 and 217 that were on file under Packt product URLs while the
candidates arrived as O'Reilly reader URLs. That is 5 of 12, and it is the fifth and sixth instance of
the same identity-key defect in one pass, which is what finally motivated the script described below.
...
| 21 | TrustLLM | **Japan Ministry of Internal Affairs and Communications** (総務省, Cybersecurity Task Force, AI Security Subcommittee) | **A NICT presentation submitted to a MIC committee.** Official material 資料５－２ for the subcommittee's 5th meeting, November 2025. Slide 7 of 12, headed 評価軸 (evaluation axes), adopts TrustLLM's six dimensions wholesale as the evaluation axes for a national AI-security evaluation platform. It glosses each dimension in Japanese and cites trustllmbenchmark.github.io. That citation is the disambiguator, and it is required: it separates this document from the registered trustllm.eu EuroHPC collision. Counted as a Tier 0 document, because the counting unit is the official document and MIC published it as its own committee material on soumu.go.jp. It is not evidence of an independent adopter. Takeshi Takahashi of NICT presented it, and the PDF author metadata names Daisuke Inoue of NICT. Takahashi also presented the symposium deck at row 20b. Row 20 is a separate NICT deck whose author this ledger does not record. What the row shows is this work reaching a ministry policy venue through NICT. | 2026-09-22 |
...
**Count: 38 government/policy/foundation-model citations, one per row** (TrustLLM x18, PyOD x13, TDC x3, DoxBench x1, COPOD x1, ADBench x1, ECOD x1). Row 21, the Japan Ministry of Internal Affairs and Communications white paper, was added 2026-09-22.
...
| G13 | SLAC National Accelerator Laboratory | MetaOD | "Coincident learning for beam-based rf station fault identification using phase information at the SLAC linac coherent light source", Physical Review Accelerators and Beams 28, 124601, December 2025. Reference 26 is MetaOD (Zhao, Rossi, Akoglu; arXiv:2009.10606), benchmarked in the methods as one of three thresholders for radio-frequency station fault detection. A separate paper from G3. The lane's "MetaOD/PyOD" claim is MetaOD alone: the 18-page PDF returns zero hits for `pyod`. Verified 2026-09-12 and promoted in that pass, but the row was not written until 2026-09-22. [OSTI 3005876](https://www.osti.gov/biblio/3005876) · DOI 10.1103/zmmr-ry9h | 2026-09-12 |

**Count: 9 DOE national-laboratory reports with substantive use, across 6 labs (PyOD x2, TrustLLM x2, HPOD, COPOD, TDC, LSCP, MetaOD), over 13 rows.** Rows G9 through G12 were appended on 2026-08-13 and fall outside that basis. G10 and G11 are DOE-hosted documents rather than laboratory reports, and G9 (arxiv.org) and G12 (pubs.rsc.org) are not government documents. They stay in the table, uncounted, until a round re-places them. G13 was promoted on 2026-09-12 with no row written, and was written on 2026-09-22.
...
| 61 | COPOD | **medium.com** | dedicated-coverage | A Towards Data Science explainer devoted to COPOD, by independent data scientist Alexandra Amidon: "Copula-Based Outlier Detection — COPOD — is a new algorithm for anomaly detection. It has been implemented in the PyOD python package." [source](https://medium.com/data-science/fast-accurate-anomaly-detection-based-on-copulas-copod-3133ce904bb1) |
| 62 | ECOD | **medium.com** | dedicated-coverage | A dedicated ECOD chapter in Chris Kuo's Medium series, with a companion notebook. The author is already represented at Ledger 3 row 38 for the book; this post is a separate artifact: "This chapter introduces a related method, Empirical Cumulative Distribution-based Outlier Detection (ECOD). These two methods are often used together. ECOD uses Empirical Cumulative Distribution Function (ECDF) to detect outliers." [source](https://medium.com/dataman-in-ai/handbook-of-anomaly-detection-3-ecod-c1b567d05bab) |
| 63 | Revisiting Time Series Outlier Detection | **medium.com** | prior-art | Sia Partners' data-science team builds its anomaly taxonomy and its diagram directly on the Lai, Zha, Xu, Zhao et al. NeurIPS 2021 definitions: "Examples of point (left), contextual (middle), and collective (right) outliers (Source: Lai & al. Revisiting Time Series Outlier Detection: Definitions and Benchmarks.)" [source](https://medium.com/sia-ai/anomaly-detection-in-time-series-3f36c7b4cd73) |
| 64 | Topology Matters: Measuring Memory Leakage in Multi-Agent LLMs | **Amazon Science** | T2 | RecSys 2026 paper by Anjun Hu (University of Oxford) with Hanting Xie, Saranya Govindan, Jas Kandola and Kurt Cutajar (Amazon, London). It reproduces and adapts MAMA from Topology Matters (arXiv:2512.04668) as one of its two extraction-driven attacks. The attack runs on AgentCF over Amazon Bedrock, with per-category PII leakage reported in Figure 4 and Table 6. Substantive reuse of the method, not a bibliography-only citation. No self-citation: none of the five authors is on the Topology Matters author block. WARNING for the coordinator: the Phase A snippet's quoted sentence does NOT appear anywhere in the PDF, and its reference-[26] author list was truncated. That sentence was 'We adopt the Multi-Agent Memory Audit (MAMA) framework [26] to quantify cross-agent leakage under varying graph topologies.' The finding is real but the Phase A quote was fabricated. Note also that the paper expands MAMA as Multi-Agent Memory Attack, not Audit. Tier demoted from the Phase A T1: amazon.science is a corporate research-publication portal with no editorial treatment, matching the existing Ledger 2 Amazon Science rows 15 and 16. | 2026 | fmco | [Link](https://www.amazon.science/publications/attacking-and-defending-multi-agent-collaborative-filtering-systems-through-connectivity) |
| 65 | ELECT: Toward Unsupervised Outlier Model Selection | **Amazon Science** | T2 | ICML 2025 paper by Saumya Gaurang Shah, Abishek Sankararaman, Balakrishnan (Murali) Narayanaswamy and Vikramank Singh, all Amazon (AWS). Stronger than the Phase A note said: it cites five Yue Zhao works (ELECT, MetaOD, ADBench, HYPER, and the SIGKDD Explorations UOMS review). It also runs MetaOD as a scored baseline in Table 5 with average precision and runtime, beyond a related-work mention. ELECT is named in body text on p9. No self-citation: none of the four authors appears on any of those author blocks. Tier demoted from the Phase A T1: amazon.science is a corporate research-publication portal, matching Ledger 2 Amazon Science precedent rows 15 and 16. ELECT has no prior ledger row; the audit's citation index currently lists ELECT under 'Not found (8)' at line 3465, which this record falsifies. | 2026 | fmco | [Link](https://www.amazon.science/publications/sead-unsupervised-ensemble-of-streaming-anomaly-detectors) |
| 66 | TrustLLM | **Amazon Science** | T2 | NAACL 2025 Industry Track paper by Aman Goel, Xian Carrie Wu, Zhe Wang, Dmitriy Bespalov and Yanjun Qi, all Amazon Web Services. TrustLLM is cited in the body to interpret the authors' own attack-success results across model sizes, not run as a benchmark. Note for the skeptic: the bib entry lists Yue Huang first and truncates at 'et al.', so Yue Zhao's name does not appear in this document. The direct mention rests on the paper title 'Trustllm: Trustworthiness in large language models' plus arXiv:2401.05561, which is the registry's required disambiguator and rules out the trustllm.eu EU project. No self-citation: Yanjun Qi and the other four authors are absent from the TrustLLM author block. Tier demoted from the Phase A T1 for the same reason as the other amazon.science rows. | 2026 | fmco | [Link](https://www.amazon.science/publications/turbofuzzllm-turbocharging-mutation-based-fuzzing-for-effectively-jailbreaking-large-language-models-in-practice) |
| 67 | TrustLLM | **Amazon Science** | T2 | Single-author paper by Daniel Lopez-Martinez (kdlopezm@amazon.com, Amazon, Santa Clara), KDD GenAI evaluation workshop. TrustLLM is the anchor citation for the paper's framing claim about LLM trustworthiness and is cited again for hallucination. The reference spells out the full title, arXiv:2401.05561 and Yue Zhao's name, so both the paper-title and PI-name disambiguators hold. Body citation, not a benchmark run, so weaker than the SEAD and RecSys rows. No self-citation: the sole author is absent from the TrustLLM author block. Tier demoted from the Phase A T1 for the same reason as the other amazon.science rows. | 2026 | fmco | [Link](https://www.amazon.science/publications/trustworthiness-in-medical-product-question-answering-by-large-language-models) |
| 68 | Yue Zhao (person) | **Carnegie Mellon University** | T2 | CMU CSD news desk covers the Meta 2022 AI4AI Research Award naming Yue Zhao as student co-PI with Zhihao Jia. The award itself is already Ledger 4 #86, so this is new university-PR coverage of a tracked award, not a new award. Identity corroborated by cv-full.tex:343 claiming the Meta 2022 AI4AI award as student co-PI and by Zhihao Jia co-authorship on TOD and BOND/PyGOD. Caveat that the article places him in the CS Department while his PhD is CMU Heinz College. | 2026 | web1 | [Link](https://www.csd.cs.cmu.edu/news/scs-team-wins-meta-award-for-work-to-lower-financial-environmental-costs-of-ai) |
| 69 | A Survey on Model Extraction Attacks and Defenses for Large Language Models | **Cloud Security Alliance** | T2 | Cloud Security Alliance Lab Space research note (2026-03-11, modified 2026-05-20, no byline) cites the Kaixiang Zhao / Yue Zhao model-extraction survey at reference [13] to carry a load-bearing claim about the limits of extraction defenses. | 2026 | standards | [Link](https://labs.cloudsecurityalliance.org/research/csa-research-note-llm-model-extraction-cloud-scale-20260308/) |
| 70 | PyOD | **Cloudera Fast Forward Labs** | T2 | Cloudera Fast Forward Labs applied research report FF12, 'Deep Learning for Anomaly Detection', gives PyOD its own named subsection under Open Source Tools and Frameworks with a description and three bullet advantages. | 2026 | standards | [Link](https://ff12.fastforwardlabs.com/) |
| 71 | Therapeutics Data Commons | **EurekAlert!** | T2 | AAAS EurekAlert! carries the Insilico Medicine and Liquid AI release announcing an ICLR paper that benchmarks LFM2-2.6B-MMAI against TxGemma-27B on the TDC benchmark. Institutional PR naming TDC as the evaluation standard. Same originator as record 9 but a different document and a different claim; both kept separately. | 2026 | web1 | [Link](https://www.eurekalert.org/news-releases/1122856) |
| 72 | TrustLLM | **Holistic AI** | T2 | Holistic AI, an AI governance and audit vendor, catalogues TrustLLM as a risk-oriented composite benchmark, states its six dimensions, its 30-plus datasets and its 16 evaluated models, and links the project repository; authored by Xin Guan, 2024-07-01. | 2026 | standards | [Link](https://www.holisticai.com/blog/navigating-llm-benchmark) |
| 73 | TrustLLM | **Linux Foundation AI & Data** | T2 | LF AI & Data (Linux Foundation) Generative AI Commons community blog lists TrustLLM as tool 35 in the Responsible Generative AI Framework v0.9 open-source tool registry, hyperlinked to the project site. | 2026 | standards | [Link](https://lfaidata.foundation/communityblog/2026/04/22/putting-rgaf-to-work-build-and-audit-responsible-ai-with-open-source/) |
| 74 | Yue Zhao (person) | **RAND Corporation** | T1 | RAND research report RRA4685-1, 'Securing AI Algorithmic Insights' (2026-07-20), cites the Kaixiang Zhao / Yue Zhao model-extraction survey in footnote 4 on printed page 42. The survey is the sole evidence for the report's Model Extraction attack vector, and it recurs in the References list on printed page 88. | 2026 | standards | [Link](https://www.rand.org/pubs/research_reports/RRA4685-1.html) |
| 75 | TrustLLM | **RBC Borealis** | T2 | RBC Borealis (RBC's AI research institute) recommends TrustLLM as the primary trustworthiness benchmark for enterprise AI adoption in finance, with a full bibliographic reference to arXiv:2401.05561. Disambiguated from trustllm.eu by the arXiv ID and the Lichao Sun author block. Bylined T. Z. Lee and J. Vitiritti, 13 Aug 2024; neither is a TrustLLM co-author, so no self-citation. | 2026 | web1 | [Link](https://rbcborealis.com/news/the-trust-factorpractical-steps-for-the-safe-adoption-of-ai/) |
| 76 | TrustLLM | **SC World** | T1 | SC Media's own editorial prose names TrustLLM alongside HarmBench and AILuminate while covering Cisco's multi-turn prompt-injection study. Disambiguated from trustllm.eu by the benchmark framing (single-turn LLM safety evaluation), which the EU Horizon project is not. TIER MOVED UP against the ceiling rule: Phase A guessed T2, but outlet-registry.md line 37 places SC Media (scworld.com) under Tier 1 Outlets, Security Press. Flagged for the skeptic. | 2026 | web1 | [Link](https://www.scworld.com/news/cisco-study-finds-major-frontier-models-susceptible-to-multi-turn-prompt-injection-attacks) |
| 77 | TrustLLM | **Transformer** | T2 | Transformer names TrustLLM in editorial prose about automated alignment research and hyperlinks it to https://github.com/HowieHwong/TrustLLM, the official benchmark repository, which rules out the trustllm.eu collision. New outlet for the audit; not covered by the 2026-07-31 AI-newsletter negative, which concerned the agent-auditing line rather than TrustLLM. | 2026 | web1 | [Link](https://www.transformernews.ai/p/ai-alignment-researchers-want-to-superintelligence) |

**Count: 121 counted items across 122 rows** (row #22 SitePoint is present but marked REMOVED and is not counted). Recounted 2026-09-22 from the table itself. The previous figure of 80 dated from 2026-08-09, and later rounds appended rows without updating it.
...
|---|-----------|------|--------|
| 37 | PyOD | **Book** | "Outlier Detection in Python" by Brett Kennedy (Manning, ISBN 9781633436473; also on O'Reilly Learning) -- Ch. 6, 7, 14 on PyOD. A video edition of the same book ships as O'Reilly 9781633436473VE; same artifact, different medium, counted once here. |
| 38 | PyOD | **Book** | "Handbook of Anomaly Detection" by Chris Kuo (Columbia) -- entire book on PyOD |
| 39 | COPOD | **Book** | "Finding Ghosts in Your Data" by Kevin Feasel (Apress, ISBN 9781484288702; also on O'Reilly Learning) -- Ch. 12 on COPOD |
...
| 288 | PyOD | **Book** | "Data Literacy with Python" (ISBN 9781501518683), Ch. 2 "Outlier and Anomaly Detection" -- list-mention, verified at Chapter 2 ('Outlier and Anomaly Detection'), page PT71 via Google Books preview: "... PyOD • Numeric Outlier (IQR) • Z-Score • DBSCAN • Isolation Forest Perform an online search for articles that discuss deep learning and anomaly detection, as well as the products that are in the preceding bullet list. WORKING. WITH." [source](https://learning.oreilly.com/library/view/data-literacy-with/9781501518683/08_Chapter_02.xhtml) |
| 289 | PyOD | **Book** | "Deep Learning and Scientific Computing with R/Python" (ISBN 9781804617755), Ch. 1 -- substantive, verified at Chapter 1, Chapter1/PyOD_point_anomaly/chapter1_pyod_point_anomaly.ipynb cells 0, 2, 5 via Packt companion repository notebook: "[PyOD](https://github.com/yzhao062/pyod), is an open-source Python library that provides over 40 outlier detection algorithms, covering from outlier ensembles to neural network-based methods on multivariate data under a unified API." [source](https://learning.oreilly.com/library/view/deep-learning-and/9781804617755/B18948_01.xhtml) |
| 290 | PyOD | **Book** | "Machine Learning for Time-Series with Python" (ISBN 9781801819626), Ch. 6 "Unsupervised Methods for Time-Series" -- list-mention, verified at Chapter 6 ('Unsupervised Methods for Time-Series'), Figure 6.6 and paragraph following Fig 6.5/6.6 via public translation mirror: "Prophet 和 PyOD 的人气都在持续上升(GitHub stars)。" [source](https://learning.oreilly.com/library/view/machine-learning-for/9781801819626/Text/Chapter_6.xhtml) |
| 291 | PyOD | **Book** | "Python Data Cleaning Cookbook" (ISBN 9781803239873), Ch. 4 -- substantive, verified at Chapter 4, 4. OutliersMultivariate/5. outliers_knn.py lines 1-3, 23 via Packt companion repository script: "# import pandas, pyod, and sklearn import pandas as pd from pyod.models.knn import KNN" [source](https://learning.oreilly.com/library/view/python-data-cleaning/9781803239873/Text/Chapter_4.xhtml) |
| 292 | PyOD / ECOD / COPOD / SUOD | **Book** | "Time Series Analysis with Python Cookbook" 2nd ed. (ISBN 9781805124283), Ch. 13 -- substantive, verified at Chapter 13, code/Ch13/Chapter_13.ipynb cells 87, 88, 100, 109, 125, 132, 136, 146, 148 via Packt companion repository notebook: "from pyod.models.ecod import ECOD # Initialize and fit the ECOD model ecod = ECOD(contamination=0.03) ecod.fit(tx)" [source](https://learning.oreilly.com/library/view/time-series-analysis/9781805124283/Text/Chapter_13.xhtml) |
| 293 | PyOD | **Book** | "Time Series with Python" (ISBN 9781805128182), Ch. 17 -- substantive, verified at Chapter 17, Chapter_17/chapter17_unsupervised_anomaly.py lines 658-664, 887, 919 via Packt companion repository script: "#\| label: pyod import # pip install pyod from pyod.models.iforest import IForest" [source](https://learning.oreilly.com/library/view/time-series-with/9781805128182/Text/Chapter_17.xhtml) |
| 294 | Agent Safety Is Action Alignment | **Curated list** | A 1,909-star hand-curated LLM-safety reading list carries the paper with title, arXiv URL and USC affiliation under Security & Discussion, at subtopic/Security&Discussion.md line 523: the entry gives the date 26.06, the affiliation University of Southern California, an arXiv link to 2606.28739, and the curated keywords action alignment, least privilege, agent safety [source](https://github.com/ydyjya/Awesome-LLM-Safety) |
| 295 | Agent Safety Is Action Alignment | **Ecosystem** | A 69-star TypeScript project encodes the paper's argument as a structured concept record, quoting its quantitative results, at .college/departments/agent-systems/concepts/security/action-authority-alignment.ts lines 4 and 20: "arXiv:2606.28739 (2026), "Agent Safety Is Action Alignment," argues that importing the chatbot-era refusal recipe — training weights to decline unsafe inputs and accepting the resulting capability loss as a manageable "alignment tax" — into the..." [source](https://github.com/Tibsfox/gsd-skill-creator) |
| 296 | SkillCenter | **Ecosystem** | A third-party Go gateway implements SkillCenter's source-grounding rule in code rather than citing it, naming the paper and arXiv ID in its research addendum, at docs/research/rsi-research-2026h2-addendum.md lines 144-147 and gateway-go/internal/domain/skills/genesis/curriculum.go lines 55, 432-440: "SkillCenter (2607.07676).** Both ground acquired skills in sources; SkillCenter's defining rule: *"each retained claim maps to an exact quotation in its source."*" [source](https://github.com/choiceoh/Deneb) |
| 297 | SkillCenter | **Ecosystem** | A 22-star Python project references SkillCenter through its repository and Hugging Face dataset handles rather than the paper, so an identifier-keyed search cannot see it, at docs/architecture/knowledge_graphs_inventory.md lines 80-88 and docs/architecture/USCODE_SPARSE_GRAPHRAG_PLAN.md line 97: the inventory names the Hugging Face dataset handles `Tommysha/skillcenter-bundles` and `Publicus/skillcenter-*` and the SkillCenter repository, with no paper identifier anywhere [source](https://github.com/endomorphosis/ipfs_datasets_py) |
| 298 | Implicit Execution Tracing | **Ecosystem** | A third-party Go project cites the paper by full title and arXiv ID as related work on multi-agent provenance, the work's only code-search footprint, at RESEARCH.md lines 111 and 168: "- **When Only the Final Text Survives: Implicit Execution Tracing for Multi-Agent Attribution** — [arXiv 2603.17445](https://arxiv.org/pdf/2603.17445) — same theme, more recent." [source](https://github.com/Stage-11-Agentics/etch/blob/main/RESEARCH.md) |
| 299 | Agent Safety Is Action Alignment | **Ecosystem** | An independent blog post analyses the paper's action-alignment argument against guardrail training, at content/blog/2026-08-18-AI-Agent-Safety.md lines 12 and 43: "Li, Shawn, and Yue Zhao. 2026. “Agent Safety Is Action Alignment.” arXiv:2606.28739. Preprint, arXiv, June 27. https://doi.org/10.48550/arXiv.2606.28739." [source](https://github.com/smirby/aiextinctionrisk) |
| 300 | PyOD | **Forum** | A Stack Overflow question and accepted answer explain PyOD's unsupervised KNN against scikit-learn's supervised classifier, by independent contributors, at Question body and answer 71847567: "kNN as an unsupervised outlier detector (as in PyOD's KNN class) measures the distance to the k nearest neighbor(s) and predicts a point to be an outlier if that distance is "large"." [source](https://stackoverflow.com/questions/71847229/the-reason-of-different-results-of-knn-algorithm-from-pyod-sklearn-packages) |
| 301 | The Autonomy Tax | **Newsletter** | Phase A dropped the /posts/ path segment. Dedicated analytical feature on the paper: names the title, both authors and USC, and links the arXiv abstract. It reports the 47-77% benign-task failure and the 81-86% obfuscation and 73-78% social-engineering bypass rates, then adds its own caveat about the 7B-8B model scale. Distinct from Ledger 2 row 36ab (The Autonomy Review) and Ledger 3 row 109. Demoted T2 to T3 under the AI-generated cap. The blog states 'I am an AI agent reading papers about AI agents - and writing about what I find', and every post is bylined MeefyBot. Quote: "Paper: "The Autonomy Tax: Defense Training Breaks LLM Agents" - Shawn Li, Yue Zhao (University of Southern California). arxiv.org/abs/2603.19423" [Link](https://latentvariable.ai/posts/the-autonomy-tax/) |
| 302 | TrustLLM | **Newsletter** | Towards Data Science computer-vision survey by Geoffrey Williams (8 Mar 2024) cites TrustLLM in its reference list with the full author block including Y. Zhao and arXiv:2401.05561. Distinct from the four TDS PyOD rows already in the ledger (L2228, L2229, L2245, L2550-2551); the author is independent of the TrustLLM co-author list. Quote: "[xxxiii] L. Sun, Y. Huang, H. Wang, S. Wu, Q. Zhang, C. Gao, ... Y. Liu, Y. Ye, Y. Cao, Y. Chen, Y. Zhao, "TrustLLM: Trustworthiness in Large Language Models," arXiv:2401.05561. 2024, 6, 7" [Link](https://towardsdatascience.com/sensemaking-at-scale-navigating-current-trends-in-computer-vision-35ca7b705ac2/) |
| 303 | PyOD | **Book** | Packt "Artificial Intelligence for IoT Cookbook" (Michael Roshak, ISBN 9781838981983) Ch.5 builds its Sense HAT / autoencoder and Isolation Forest IoT anomaly-detection recipes directly on PyOD. Companion repo imports pyod in five files and links github.com/yzhao062/pyod in a source comment. Phase A T2 lowered to T3; books sit in Ledger 3 alongside rows 66e, 195, 289, 291-293. Quote: "from pyod.models.auto_encoder import AutoEncoder" [Link](https://www.packtpub.com/en-us/product/artificial-intelligence-for-iot-cookbook-9781838981983) |
| 304 | TDC | **Book** | The Life Sciences AI Handbook (Bryan Tegomoh, MD MPH, independent single author, not a TDC co-author) gives Therapeutics Data Commons a named benchmark subsection with substantive body prose. It cites Huang et al. 2022 Nat Chem Biol, whose Crossref author block is Kexin Huang; Tianfan Fu; Wenhao Gao; Yue Zhao; Yusuf Roohani; Jure Leskovec; Connor W. Coley; Cao Xiao; Jimeng Sun; Marinka Zitnik. Phase A T2 lowered to T3: an open-access handbook is a book. Quote: "The Therapeutics Data Commons (Huang et al., 2022) extended this principle to a broad multi-task benchmark covering small molecules, antibodies, vaccines, gene editing, and clinical trials, with biology-aware data splits (scaffold, time, target) and a shared l" [Link](https://lifesciencesaihandbook.com/therapeutics/small-molecules.html) |
| 305 | PyOD | **Platform** | Anomstack (Dagster + FastHTML metrics anomaly app) names PyOD as its ML engine and links yzhao062/pyod in the README. The same author, Andrew Maguire, is already in the ledger at rows #71 and #101 for two DIFFERENT projects (a 2023 blog post and airflow-provider-anomaly-detection), so count the project, not a new adopter. Phase A T2 demoted to T3: a single-maintainer OSS app is a platform integration, the Ledger 3 class. Quote: "1. ⭐ - You bring your metrics Anomstack will do the ML (❤[PyOD](https://github.com/yzhao062/pyod))." [Link](https://github.com/andrewm4894/anomstack) |
| 306 | PyOD | **Platform** | NEW distro packaging: the Arch User Repository carries python-pyod 2.0.6-1 tracking the upstream tag, and two AUR packages depend on it, one of them python-tods-git (TODS, another Yue Zhao work). Weak-signal caveats belong in the row: templated database page, Maintainer: None, 0 votes, 0.000000 popularity, flagged out-of-date 2026-03-08, and the licence field says GPL3 where PyOD is BSD 2-Clause. Phase A T3 holds. Quote: "Package Base: python-pyod Description: A Python 3 Library Outlier Detection or Anomaly Detection Upstream URL: https://github.com/yzhao062/pyod" [Link](https://aur.archlinux.org/packages/python-pyod) |
| 307 | When Only the Final Text Survives (Nian et al.) | **Platform** | AGGREGATOR. The repo's own README advertises 'Daily Update enabled' linking .github/workflows/daily-arxiv-update.yml, so the entry is a machine arXiv crawl rather than a curation decision. Per disclaimer-patterns that caps it at T3, and the adoption signal is close to zero. Title variant 'for Multi-Agent Attribution' against data/publications.json's 'for Multi-Agent Auditing', the same variant the 2026-09-12 pass recorded at lines 487-489; same arXiv ID. Quote: "- **2026-03-18** [arXiv ID: 2603.17445] [When Only the Final Text Survives: Implicit Execution Tracing for Multi-Agent Attribution](http://arxiv.org/abs/2603.17445v2)" [Link](https://github.com/xaddwell/Awesome-LLM-Agent-Memory-Safety) |
| 308 | PyOD | **Platform** | The largest third-party ML catalogue on GitHub (74.4K stars, pushed the day of this audit) carries a dedicated PyOD entry linking yzhao062/pyod. No prior verdict for this repo in any round's records. Strongest of the three curated-list finds in this shard. Quote: "* [PyOD](https://github.com/yzhao062/pyod) -> Python Outlier Detection, comprehensive and scalable Python toolkit for detecting outlying objects in multivariate data. Featured for Advanced models, including Neural Networks/Deep Learning and Outlier Ensembles." [Link](https://github.com/josephmisiti/awesome-machine-learning/blob/master/README.md) |
| 309 | PyOD | **Platform** | Four of this lab's repos in one third-party list (pyod, combo, anomaly-detection-resources, awesome-ensemble-learning). Zhining Liu does not appear anywhere in data/publications.json, so this is not a co-author listing. Weaker than the josephmisiti row: 841 stars and no push since 2023-03-08. Quote: "- [**[Library] pyod**](https://github.com/yzhao062/pyod) ![](https://img.shields.io/github/stars/yzhao062/pyod?style=social) - A Python Toolbox for Scalable Outlier Detection (Anomaly Detection)." [Link](https://github.com/ZhiningLiu1998/awesome-machine-learning-resources) |
| 310 | TDC | **Platform** | Third-party curated chemistry-software list (1,442 stars); maintainer Lukasz Mentel is not a co-author. Full name 'Therapeutics Data Commons' plus the mims-harvard/TDC link clears the TDC acronym collision. Held at the Phase A T4 ceiling, though by content it is the same class as the T3 awesome-list row above. The three curated lists in this shard arrived with inconsistent Phase A tiers (T3/T4/T4) and should be normalised to one tier in the ledger. Quote: "- [TDC](https://github.com/mims-harvard/TDC) - Therapeutics Data Commons (TDC) is the first unifying framework to systematically access and evaluate machine learning across the entire range of therapeutics." [Link](https://github.com/lmmentel/awesome-python-chemistry) |
| 311 | Agent Banana (Ye et al.) | **Platform** | Hand-curated NJU list carries Agent Banana with full title, arXiv PDF link, project page, code link and the TAMU attribution. Distinct from the Agent Banana aggregator paper-pages already dismissed at lines 2020 and 2668. Phase A T3 holds. Note the README path is lowercase readme.md; the capitalised path 404s. Quote: "/2026/ArXiv/**Agent Banana**: High-Fidelity Image Editing with Agentic Thinking and Tooling/[paper](https://arxiv.org/pdf/2602.09084)[project page](https://agent-banana.github.io/)[code pending](https://github.com/taco-group/agent-banana)/TAMU/" [Link](https://github.com/njulj/Awesome-Agent-Based-Low-Level-Vision) |
| 312 | PyOD | **Platform** | The strongest new find in this shard. Diogo Ribeiro (ORCID 0009-0001-2022-7072, Faculty of Media Arts and Design, Technical University of Porto), no co-author overlap. AnomalyBench declares `pyod` and `combo` as core dependencies, cites the PyOD JMLR paper as its first formal reference, and wraps PyOD classes through a dedicated `_PyODAdapter`. Notable because external `combo` adopters are rare, and because ROADMAP.md records a 2026-07-22 review 'against the current PyOD capability catalogue'. Quote: "Zhao, Y., Nasrullah, Z., & Li, Z. (2019). PyOD: A Python Toolbox for Scalable Outlier Detection. Journal of Machine Learning Research, 20(96), 1-7." [Link](https://doi.org/10.5281/zenodo.21496904) |
| 313 | PyOD | **Platform** | MIRROR, AND THE PHASE A ATTRIBUTION IS WRONG. The file is byte-identical to netdata/netdata v1.37.1 docs/guides/monitor/anomaly-detection-python.md (diff run, zero differences; upstream front matter reads author: Joel Hans, Netdata). ETH Zurich ISG Physics runs a clone of the Netdata repo and wrote nothing here. The real, previously unrecorded signal is Netdata's own PyOD-powered anomalies collector, absent from every ledger (zero hits for 'netdata' in news-coverage-audit.md). Record it as Netdata, cite the upstream path, and flag aggregator / verbatim-mirror, which caps it at T3. Quote: "Netdata's implementation uses a handful of functions in the [Python Outlier Detection (PyOD) library](https://github.com/yzhao062/pyod/tree/master), which periodically runs a `train` function that learns what normal looks like" [Link](https://gitlab.phys.ethz.ch/isgphys/netdata/-/blob/v1.37.1/docs/guides/monitor/anomaly-detection-python.md) |
| 314 | SocialMaze: A Benchmark for Evaluating Social Reasoning in Large Language Models | **Platform** | COBRA-Skills (arXiv:2609.11682) vendors a fixed subset of SocialMaze's hard split as one of six benchmark adapters and attributes it by name, first author and institution. Its authors are Pingchen Lu, Xiangyi Wang, Xiang Li, Jie Mao, Zikun Qu, Junfeng Luo, Yao Shu, Bryan Kian Hsiang Low, Zhongxiang Dai, with no overlap with the SocialMaze author block, so not self-citation. Phase A T3 holds. Quote: "- Project: SocialMaze, by Zixiang Xu et al., Mohamed Bin Zayed University of Artificial Intelligence." [Link](https://github.com/Jerry-LuP/COBRA-Skills) |
| 315 | GLIP-OOD: Zero-Shot Graph OOD Detection with Graph Foundation Model | **Platform** | Awesome-Foundation-Models-on-Graphs (survey companion list) leads its graph anomaly-detection section with GLIP-OOD, title verbatim plus arXiv link. Distinct from the only prior GLIP-OOD row, the Quantum Zeitgeist blog at #24. Phase A T3 holds. Quote: "- [Arxiv 25.04] **GLIP-OOD: Zero-Shot Graph OOD Detection with Graph Foundation Model** [[PDF](https://arxiv.org/abs/2504.21186)]" [Link](https://github.com/Zehong-Wang/Awesome-Foundation-Models-on-Graphs) |
| 316 | PyOD | **Platform** | NEW: the TimeEval time-series anomaly-detection benchmark suite (Hasso Plattner Institute, maintainer sebastian.schmidl@hpi.de) publishes a dedicated ghcr.io/timeeval/pyod intermediate container image pinned to pyod==1.1.2, which every PyOD-backed algorithm image in the suite builds on. Zero prior TimeEval or HPI rows in the ledger. Phase A T2 demoted to T3 (container image = platform integration). Quote: "# install pyod library and cleanup afterwards RUN set -eux; \ pip install --no-cache-dir pyod==1.1.2; \" [Link](https://github.com/TimeEval/TimeEval-algorithms/blob/main/1-intermediate-images/pyod/Dockerfile) |
| 317 | PyOD | **Platform** | Real but the lowest-value item in this shard: a personal model card with 0 downloads and 0 likes, created 2026-05-11. Merge into a Hugging Face long-tail platform row rather than giving it a row of its own. Quote: "LightGBM 10-class disconnect classifier + PyOD anomaly detector + reproducible synthetic-data generator for the [AI Internet Diagnostic](https://github.com/WolfDavid/ai-internet-diagnostic-space) project." [Link](https://huggingface.co/WolfDavid/ai-internet-diagnostic-model) |
| 318 | PyOD | **Platform** | HybridQC, an R package for scRNA-seq quality control, calls PyOD's Isolation Forest through reticulate and declares pyod a hard dependency. Two cautions for the row. The repo is tiny (0 stars, created 2025-07-03, last push 2025-07-15, fork: false). Its own README points Source Code at github.com/biosciences/HybridQC, which returns Not Found from the GitHub API, so taowis/HybridQC is the live copy. Phase A T2 demoted to T3. Quote: "reticulate::import("pyod.models.iforest")" [Link](https://github.com/taowis/HybridQC) |
| 319 | TDC | **Platform** | K-Dense AI ships a dedicated PyTDC agent skill (install pins PyTDC 1.1.15, names mims-harvard/TDC as the source repo, and instructs users to cite TDC). External to the TDC team. Phase A guessed T4; T4 in this audit is awards and encyclopedia entries, so this is recategorised one step to T3, the platform-integration class, and is a category fix rather than an authority upgrade. Quote: "Use Therapeutics Data Commons through the PyTDC Python package for registry discovery, approved dataset access, task-aware splits, evaluator metrics, benchmark groups, and bounded molecular-oracle workflows." [Link](https://raw.githubusercontent.com/K-Dense-AI/claude-scientific-skills/main/skills/pytdc/SKILL.md) |
| 320 | PyOD | **Platform** | NEW book row: Deep Learning for Time Series Cookbook (Cerqueira and Roque, Packt) devotes three Chapter 9 recipes to PyOD (autoencoder, VAE, GAN) and pins pyod==1.1.2 in requirements.txt. Joins ledger book rows #290 to #293. The README never says PyOD, so cite the chapter files, not the README. Phase A T2 demoted to T3 (book, the Ledger 3 class). Quote: "from pyod.models.auto_encoder_torch import AutoEncoder" [Link](https://github.com/PacktPublishing/Deep-Learning-for-Time-Series-Data-Cookbook) |
| 321 | TrustLLM | **Platform** | CMedBench (compressed medical LLM benchmark) vendors the whole TrustLLM toolkit as its Track 3 trustworthiness track. The vendored TrustLLM/README.md in the same repo links arXiv:2401.05561 and trustllmbenchmark.github.io, which rules out the trustllm.eu EU-project collision in the disambiguation registry. Phase A T3 holds. Quote: "cd TrustLLM/trustllm_pkg pip install -e ." [Link](https://github.com/Tabrisrei/CMedBench/blob/master/README.md) |
| 322 | PyOD | **Platform** | Two hits in one thread, both from people outside the lab: a scikit-learn contributor points a user at PyOD's KNN detector, and the issue author cites ADBench's arXiv PDF. Phase A called albertcthomas the 'triage team'; the API reports author_association CONTRIBUTOR, so the row should say contributor. Same class as the existing Stack Overflow row, Ledger 3 #300. Quote: "@PierrickPochelu you might also want to try the pyOD package which implements a kNN based outlier detector https://pyod.readthedocs.io/en/latest/pyod.models.html#module-pyod.models.knn" [Link](https://github.com/scikit-learn/scikit-learn/issues/24574) |
| 323 | PyOD | **Platform** | NEW: sktime ships a first-class PyODDetector adapter wrapping any PyOD estimator for time-series detection, with ECOD as the documented example. No prior sktime row exists; the ledger's two sktime hits (lines 1333, 2107) are the OpenAI job posting that lists sktime alongside PyOD 2.0. Phase A T1 is wrong by the rubric: a framework adapter is a platform integration, Ledger 3, so T3. Quote: "Transformer that applies outlier detector from pyOD. Parameters: estimator PyOD estimator See https://pyod.readthedocs.io/en/latest/ documentation for a detailed description of all options." [Link](https://www.sktime.net/docs/api-reference/sktimedetectionadapterspyoddetector/) |
| 324 | PyOD | **Course** | Georgia Tech CSE6040 "Computing for Data Analytics" pins pyod==0.8.6 in the official Vocareum student computing environment, so every enrolled student gets PyOD preinstalled. Templated requirements dump, no editorial content: T3, and the sibling pyodbc line on the page confirms the disambiguation rather than defeating it. Quote: "pyod==0.8.6" [Link](https://cse6040.gatech.edu/active/Guides/installed_modules/) |
| 325 | PyOD | **Government** | C4IIoT Horizon 2020 deliverable D3.1 'Behavioural analysis and cognitive security framework' (SU-ICT-2018, grant 833828, public, 31/05/2020), editors Milos Savic and Srdjan Skrbic (University of Novi Sad). PyOD is wrapped into a named module family (PyODAD) inside the project's anomaly-detection package. Tiered T3 rather than Phase A's T2 to match the audit's own precedent for EU Horizon deliverables, Ledger 3 row 66cc (SEDIMARK D3.1/D5.2). Registry note: every 'Aegis' hit across the C4IIoT corpus is AEGIS IT RESEARCH UG, a German consortium partner, not arXiv:2603.12621. Quote: "BACSPY providing anomaly detection based on outlier detection, classification and representation learning algorithms implemented in Python using Tensorflow 2, scikit-learn and PyOD libraries." [Link](https://ec.europa.eu/research/participants/documents/downloadPublic?documentIds=080166e5cfa5e261&appId=PPGMS) |
| 326 | PyOD | **Government** | C4IIoT Horizon 2020 deliverable D4.2 'C4IIoT Minimum Viable Product' (grant 833828, public, 31/05/2020), editors Spyridon Vantolas and Mohammad Hamad (AEGIS IT Research UG). A single cross-reference to D3.1 rather than independent use; if the coordinator collapses the C4IIoT cluster into one row, this is the one to fold in. Quote: "In this use case, BACS Edge is able to perform anomaly detection using any of the methods available in the BACSPY package of BACS (Python-based implementations of anomaly detection methods relying on scikit-learn, PyOD and Tensorflow libraries, for more detail" [Link](https://ec.europa.eu/research/participants/documents/downloadPublic?documentIds=080166e5cfd6b621&appId=PPGMS) |
| 327 | PyOD | **Government** | C4IIoT Horizon 2020 deliverable D3.3 'Level-2 and Level-3 security mechanisms of C4IIoT' (grant 833828, public, 30/11/2020), editor Gilad Ezov (IBM). PyOD earns its own acronym-table entry and five PyOD detectors are benchmarked with timing numbers. Strongest of the four C4IIoT documents. Quote: "BACSPY is a BACS package that provides anomaly detection based on outlier detection, classification and representation learning algorithms implemented in Python using Tensorflow 2, scikit-learn and PyOD libraries." [Link](https://ec.europa.eu/research/participants/documents/downloadPublic?documentIds=080166e5d673d7ff&appId=PPGMS) |
| 328 | PyOD | **Government** | C4IIoT Horizon 2020 deliverable D3.4 'Cyber assurance and protection in an industrial cloud infrastructure' (grant 833828, public, 30/11/2021), editor Patrizia Ciampoli (HPE). PyOD detectors are scored in the deliverable's own results tables rather than only named. Quote: "PKI Public Key Infrastructure PYOD Python Outlier Detection RA Registration Authority" [Link](https://ec.europa.eu/research/participants/documents/downloadPublic?documentIds=080166e5e531978f&appId=PPGMS) |
| 329 | Therapeutics Data Commons | **Government** | 'A Meta-model for ADMET Property Prediction Analysis', Sarala Padi, Antonio Cardone and Ram D. Sriram, all National Institute of Standards and Technology, Gaithersburg (corresponding sarala.padi@nist.gov), NIST pub_id 936880, catalogued 2023-12-07. TDC is the evaluation substrate for the whole paper: 22 ADMET tasks, meta-model ranked against the TDC leaderboard. Self-citation screened clean against the TDC author block (Huang, Fu, Gao, Zhao, Roohani, Leskovec, Coley, Xiao, Sun, Zitnik), no overlap. The TDC coauthorship-dilution annotation the ledger applies to other TDC rows applies here too. Caveat for the skeptic: NIST's own catalog types this as 'Websites' and BioArxiv. The PDF also carries the bioRxiv CC-BY preprint banner ('It is made available under a preprint (which was not certified by peer review)'). It is therefore NIST-authored research output rather than a NIST Special Publication or a policy document. T1 held at the Phase A ceiling on that basis. Quote: "The Therapeutics Data Commons (TDC) [39] is the first unifying platform for systematically assessing and evaluating machine learning models across the entire range of therapeutics." [Link](https://tsapps.nist.gov/publication/get_pdf.cfm?pub_id=936880) |
| 330 | PyOD | **Government** | PyChemAuth chemometrics toolkit documentation. Corrects the Phase A source attribution: the project is github.com/mahynski/pychemauth, a personal repository of Nathan A. Mahynski, not a usnistgov organisation repository, and the page is hosted on readthedocs.io, so NIST is the author's employer rather than the publisher. A downstream package instructing its users to cite the PyOD JMLR paper is a platform integration, the Ledger 3 class, so T3 rather than Phase A's T2. Independent of the already-counted NIST 'Science ex Machina' row even though both trace to Mahynski. Quote: "If you use PyOD be sure to cite: @article{zhao2019pyod, author = {Zhao, Yue and Nasrullah, Zain and Li, Zheng}, title = {PyOD: A Python Toolbox for Scalable Outlier Detection}, journal = {Journal of Machine Learning Research}, year = {2019}, volume = {20}, num" [Link](https://pychemauth.readthedocs.io/en/latest/citation.html) |
| 331 | PyOD | **Registry** | MCP Agents Market ('The open directory for MCP servers, skills, plugins, and agents') carries a dedicated PyOD skill listing. Its About / Key Features / Use Cases / FAQ text is written by the marketplace rather than mirrored from the README. That text correctly describes the od-expert skill, the 10-tool MCP server, and ADEngine. Templated marketplace page, so the aggregator cap applies. Tier held at the Phase A ceiling T4 although the page is the content peer of the T3 hvtracker.net and skillsllm.com rows. Flagging the inconsistency for the skeptic rather than moving the tier up. Bogus-slug control returns HTTP 404, so the page is real. Distinct from 'MCP Market' (mcpmarket.com) already inside Ledger 3 cluster row 66bt for agent-audit. Quote: "PyOD is a comprehensive Python library for anomaly detection that includes AI agent integration through the od-expert skill and MCP server." [Link](https://mcpagentsmarket.com/skill/pyod) |
| 332 | LangSkills | **Registry** | OpenClaw Skills Store (all4openclaw.ai) does list LangSkills (LabRAI/LangSkills, an open-source.json entry where Yue Zhao is Collaborator) under category 'Learning & Growth'. The store's own JSON API confirmed it after the Phase A URL proved to be a soft-200 SPA catch-all, so cite /skill/skill_ea0e390a, not /skills/langskills. All listing copy is store-written bilingual ZH/EN marketing text (description, full_description, use_cases, features all exist in both languages and are not the repo README). Treat it as machine-generated aggregator copy and cap at Tier 3. Its store heat is 1170 with user_rating 0.0 from 0 ratings: a thin listing on a real but low-traffic third-party store. Quote: "LangSkills is a massive 'AI Skill Store' that aggregates expert knowledge from over 62,000 research papers and 23,000 technical sources. It acts as a reliable knowledge base for your AI, ensuring it provides accurate, evidence-backed answers instead of guessin" [Link](https://all4openclaw.ai/skill/skill_ea0e390a) |
| 333 | anywhere-agents | **Registry** | SkillsLLM third-party skill directory carries a dedicated anywhere-agents listing with its own automated security scan, 'Verified' badge, tag set, and three head-to-head comparison pages (vs hermes-agent, vs ecc, vs everything-claude-code). Caveat for the row: the bulk of the page body is a verbatim mirror of the repo README, so the 'Yue Zhao' and 'USC CS faculty' strings are first-party text; only the listing wrapper is third-party. The site's own /ai-transparency page discloses that it 'may use artificial intelligence and other automated tools to assist with research, summarization, drafting, categorization, metadata generation, security analysis'. That is an AI-assisted flag, and it caps the row at Tier 3 independent of domain. Page is real: a bogus slug returns a content-free 200 shell while this URL returns 264 KB of listing content. Distinct from the awesomeskills.dev anywhere-agents listing named in the 2026-09-12 round summary table (line 1918). Quote: "anywhere-agents / by yzhao062 / Verified / One config to rule all your AI agents: portable (every project, every session), effective (curated writing, routing, skills), and safer (destructive-command guard). / 178 stars / 18 forks / Python / Added 4/22/2026" [Link](https://skillsllm.com/skill/anywhere-agents) |
| 334 | TDC | **Registry** | TrueFoundry Skills Registry (registry backed by github.com/truefoundry/tfy-skills-repo, 'Submit a skill' contribution flow) catalogs a pytdc agent skill that wraps Therapeutics Data Commons for drug-discovery agents. The skill ships in K-Dense-AI/scientific-agent-skills (independent org, 46.1k stars, not a fork of any lab repo, GitHub API checked), so the listing is third-party ecosystem adoption of TDC, not first-party. Templated registry page: the Summary is the skill's own frontmatter description and no author, arXiv ID or tdcommons.ai link appears, so the only disambiguator is the full name 'Therapeutics Data Commons', which the registry accepts. Bogus-slug control returns HTTP 404, so the page is real, not a catch-all. No AI-generated disclaimer on the page. Quote: "Therapeutics Data Commons. AI-ready drug discovery datasets (ADME, toxicity, DTI), benchmarks, scaffold splits, molecular oracles, for therapeutic ML and pharmacological prediction." [Link](https://www.truefoundry.com/skills-registry/skill/K-Dense-AI-claude-scientific-skills-pytdc) |
| 335 | ECOD | **Patent** | CN119937461A (AVIC Hongchuang Technology (Tianjin) Co Ltd, published 2025-05-06). Substantive: ECOD is the joint-distribution scoring stage of a multi-sensor monitoring system. Phase A title and snippet were fabricated: the real title is 'Monitoring state evaluation method and system for machining machine tools based on multi-source sensors'. The worked case is a gear-processing machine tool, not aircraft components or flight parameters. Quote: "And the joint distribution function is ECOD algorithm, and the intelligent alarm of abnormal state of the gear processing machine tool is realized. ECOD is an unsupervised anomaly detection method that identifies points of anomalies in process data based on an" [Link](https://patents.google.com/patent/CN119937461A/en) |
| 336 | ECOD | **Patent** | CN121814397A (AsiaInfo Technologies (Chengdu) Inc, published 2026-04-07). Substantive: ECOD is the primary IP-scoring detector for cyberattack origin tracing, reproducing the ECOD paper's left-tail / right-tail / auto score formulation. The drafter mis-expands the acronym as 'Extended Isolation Forest with Copula-Based Outlier Detection' but then describes Li-Zhao et al.'s ECDF method exactly. Phase A title was wrong: the patent covers attack attribution, not generic 5G-slice anomaly detection. Quote: "Illustratively, taking the ECOD algorithm as an example, ECOD is an unsupervised outlier detection algorithm based on an empirical cumulative distribution function (EMPIRICAL CUMULATIVE DISTRIBUTION FUNCTION, ECDF). The method combines outliers of the same sam" [Link](https://patents.google.com/patent/CN121814397A/en) |
| 337 | COPOD | **Patent** | CN115567572B (Beijing Baidu Netcom Science and Technology Co Ltd, granted 2025-05-27). Substantive: COPOD is one of three tabular detectors scoring longitudinal and transverse contrast features in Baidu's object-anomaly pipeline. Phase A title and snippet were fabricated: the patent is 'Method, device, equipment and storage medium for determining abnormality of an object', not an abnormal-log detection method. Quote: "Therefore, a copod model is also used in the embodiment, and the model jointly models all the features through a copula probability function, so that the relations among different features are fully considered. The model has better detection effect when carryi" [Link](https://patents.google.com/patent/CN115567572B/en) |
| 338 | ECOD | **Patent** | CN116633666A (Beijing Volcano Engine Technology Co Ltd / ByteDance, published 2023-08-22). Substantive: ECOD selects the candidate anomalous-object set that deviates from the group access baseline in an enterprise-office-network intrusion scenario. Quote: "For example, the ECOD outlier detection algorithm based on statistics can be used to determine the set of candidate abnormal objects that deviate from the population baseline, that is, the outliers can be obtained by summarizing the tail probabilities." [Link](https://patents.google.com/patent/CN116633666A/en) |
| 339 | COPOD | **Patent** | CN118798628A (China Mobile Communications Group Co Ltd + China Mobile Group Design Institute Co Ltd, published 2024-10-18). Substantive: the ICT supply-chain risk engine is an explicitly modified COPOD, computing left-tail probability for positive indicators and right-tail for negative ones and scoring against a COPOD anomaly threshold T. Quote: "The COPOD (Copula-based Outlier Detector) algorithm is an anomaly detection method based on statistical probability functions. It supports efficient data modeling. The COPOD algorithm does not require model training, has optimal detection efficiency, does not " [Link](https://patents.google.com/patent/CN118798628A/en) |
| 340 | COPOD | **Patent** | CN117113250B (Dalian University of Technology, granted 2026-02-13). Substantive and claimed: COPOD supplies the intrinsic-reward signal of the reinforcement-learning anomaly-detection agent for fine chemical production processes. Quote: "Specifically, for the intrinsic reward function, an unsupervised algorithm COPOD is introduced, and the agent exploration environment is guided through the detection result score (s t ) of the unsupervised algorithm." [Link](https://patents.google.com/patent/CN117113250B/en) |
| 341 | ECOD | **Patent** | CN120671044A (Hangzhou Dianzi University, published 2025-09-19). Substantive plus baseline-with-numbers: the invention integrates ECOD into the BikNN score and benchmarks against it in accuracy and ROC-AUC tables. It expands the acronym with the exact title of the ECOD paper (Li, Zhao et al., TKDE 2022). Quote: "In view of the above problems, the present invention provides a novel method for detecting abnormal data, which aims to provide simple abnormality interpretability, and can also integrate ECOD (Unsupervised Outlier Detection Using Empirical Cumulative Distribu" [Link](https://patents.google.com/patent/CN120671044A/en) |
| 342 | ECOD | **Patent** | CN117453502A (Institute of Software, Chinese Academy of Sciences, published 2024-01-26). Baseline-with-numbers, and stronger than Phase A stated: the patent benchmarks TWO Zhao works, ECOD and LSCP, in Table 3 (plus LUNAR), rather than ECOD alone. Real title: 'A cross-architecture fine-grained operating system performance anomaly mining method and device'. Quote: "In this example, a total of 366 pairs of cross-architecture contexts were successfully matched. Anomaly detection algorithms such as LOF, LUNAR, ECOD, SO_GAAL, and LSCP were selected to apply to the matching results. ... The description and results of each alg" [Link](https://patents.google.com/patent/CN117453502A/en) |
| 343 | MetaOD | **Patent** | CN116167004A (Jiangsu Fangtian Power Technology Co Ltd, published 2023-05-26). Substantive plus named citation: the power-data model-selection method is built on MetaOD and the patent's only non-patent citation is Yue Zhao et al.'s NeurIPS 2021 MetaOD paper. Strongest row in this shard: it names both the tool and the PI. The ledger has no prior MetaOD patent row. Quote: "YUE ZHAO等: " Automatic unsupervised outlier model selection ", 《NIPS'21: PROCEEDINGS OF THE 35TH INTERNATIONAL CONFERENCE ON NEURAL INFORMATION PROCESSING SYSTEMS》, no. 343, 6 December 2021 (2021-12-06), pages 3 - 8" [Link](https://patents.google.com/patent/CN116167004A/en) |
| 344 | ECOD | **Patent** | EP4451150A1 (Sentryonics Lda, published 2024-10-23). Named-as-usable-model: ECOD is listed among the tabular detectors the claimed behaviour-analytics engine may run. Weaker than Phase A stated: COPOD has zero occurrences in the document, so the 'ensemble of ECOD, COPOD and IForest' snippet is not in the patent. Its real title is 'Apparatus and method for detecting and mitigating anomalous access in networked systems', not an ICS cyber-threat patent. Quote: "In an embodiment, the machine learning model for the behaviour analytics is selected from a list comprising: an unsupervised machine-learning model for this task, ranging from classical tabular based methods such as k-Nearest-Neighbours, Isolation Forest, ECOD" [Link](https://patents.google.com/patent/EP4451150A1/en) |
| 345 | COPOD | **Patent** | CN118606861A (South China University of Technology, published 2024-09-06). Substantive: COPOD is one of three base classifiers in the voting ensemble that labels anomalous building-room energy-consumption data. Phase A title and snippet were wrong: the patent covers scientific-research building rooms via ensemble plus deep learning, not a copula-model smart-building method. Quote: "The COPOD algorithm carries out joint modeling on the distribution of all the features by means of copular functions, fully considers the correlation among all the features, merges the abnormality degree of the data on all the features, and outputs an abnormal" [Link](https://patents.google.com/patent/CN118606861A/en) |
| 346 | Yue Zhao (person) | **Patent** | CN115293481A / CN115293481B (Wuxi Power Supply Co of State Grid Jiangsu Electric Power Co Ltd, published 2022-11-04). The deepest patent adoption in this shard: Zhao et al.'s LSCP is named in the title and every claim, expanded in full, and used with four base detectors (iForest, OC-SVM, COPOD, LOF) to find mis-filed customer-to-transformer records. Phase A title was wrong (it is customer-transformer relationship checking, not transformer operating-state evaluation) and the snippet's 'formulated by Zhao et al.' phrasing is not in the document, which never names Zhao. Quote: "In order to solve the deficiencies in the prior art, the purpose of the present invention is to provide a method and system for checking the relationship between households in the station area based on the integrated learning LSCP (Local Selective Combination " [Link](https://patents.google.com/patent/CN115293481A/en) |
| 347 | COPOD | **Patent** | CN113761292B (Tencent Technology (Shenzhen) Co Ltd, granted 2024-12-03). Substantive: COPOD is named with its full expansion as a selectable weak learner and is given concrete tuned parameters in the worked embodiment. Quote: "Specifically, the anomaly detection model may be at least one of a random forest, an isolated forest (iForest), HBOS (Histogram-based Outlier Score, histogram-based anomaly score), COPOD (Copula-Based Outlier Detection, coupled anomaly detection-based), auto E" [Link](https://patents.google.com/patent/CN113761292B/en) |
| 348 | TDC | **Science press** | Kim, Chang, Ji and Joung (ACS JCIM, 2024) benchmark against TDC's ADMET group and name it by full title. Held at the Phase A T3 ceiling, but note the class: this is a repository record for a peer-reviewed article's supporting information, so the substance belongs with the peer-reviewed-use ledger (Ledger 7) rather than with science press. Neither 'Quantum-Informed' nor 'jcim' nor '4c00772' appears in the audit file. Quote: "The implementation of our pretraining strategy and multitask learning outperforms conventional methods, achieving state-of-the-art outcomes in 7 out of 22 ADMET tasks within the Therapeutics Data Commons by utilizing a shared encoder across all tasks." [Link](https://api.figshare.com/v2/articles/26097787) |
| 349 | TDC | **Science press** | Insilico Medicine engineering blog runs a full train/test leakage audit on the TDC Pgp Inhibition benchmark, treating TDC scaffold splits as the object of study (852 train / 245 test structures). Substantive third-party benchmark use. Tiered down from the Phase A T2 because this is a company blog case study rather than press or institutional PR. Quote: "In this case study, we demonstrate how Claude Code (powered by Claude Opus 5) connects to Insilico Medicine's Cheminformatics Engine, MolTools, via the Model Context Protocol (MCP) to automate an end-to-end train–test leakage audit on the Therapeutics Data Com" [Link](https://insilico.com/blog/model-validation) |
| 350 | PyOD | **Platform** | AI Alliance technical blog on data cleaning for LLM fine-tuning and RAG cites the PyOD paper title and the pyod.readthedocs.io docs URL at reference 5, and misattributes authorship to 'Zhaohui Wang et al.' rather than Zhao et al. Quote: "5. Zhaohui Wang et al., “PyOD: A Python Toolbox for Scalable Outlier Detection” ( https://pyod.readthedocs.io )." [Link](https://thealliance.ai/blog/mastering-data-cleaning-for-fine-tuning-llms-and-r) |
| 351 | PyOD | **Thesis** | Aalto University master's thesis (Space Science and Technology / SpaceMaster), Robert-Mihai Olariu, 24 July 2026, real telemetry from OHB Sweden. PyOD 1.0.6 supplies the KNN baseline and reports precision/recall/F1/ROC-AUC/AP against the author's ensemble. Quote: "KNN is implemented in the PyOD library [26] and serves as a transparent unsupervised baseline." [Link](https://aaltodoc.aalto.fi/handle/123456789/146815) |
| 352 | COPOD | **Thesis** | Charles University master's thesis, Ing. Petra Kohoutova, Institute of Economic Studies, defended 2024, supervisor Ladislav Kristoufek. COPOD is one of three models implemented and benchmarked for AML transaction monitoring; 68 COPOD hits including confusion matrices, ROC curves and runtime comparisons across three scenarios. Quote: "The last chosen model for our analysis is COPOD, which we will implement using the COPOD package." [Link](https://dspace.cuni.cz/handle/20.500.11956/194829) |
| 353 | PyOD | **Thesis** | Kiel doctoral thesis, Maximilian Huenemoerder, 2023, real title 'Advances in Unsupervised Learning and Applications' (the shard title is a placeholder). PyOD supplies the implementations for the thesis experiments. Recovered through the Wayback Machine because the live host runs an Anubis proof-of-work gate. Quote: "Unless noted otherwise, PyOD's [34] implementation is used." [Link](https://macau.uni-kiel.de/servlets/MCRFileNodeServlet/macau_derivate_00004993/huenemoerder_dissertation.pdf) |
| 354 | PyOD | **Thesis** | Chulalongkorn University MSc thesis (Faculty of Science), Phichapop Changsakul, 2021, DOI 10.58837/CHULA.THE.2021.115. PyOD supplies the three comparison baselines (LOF, kNN, FastABOD) against the author's own parameter-free scorers. Quote: "The LOF, kNN, and FastABOD algorithms are imported from Pyod [46] while the OOF, AOF, and MOF algorithms are coded by the author and accelerated by Numba [47]." [Link](https://digital.car.chula.ac.th/chulaetd/4657) |
| 355 | PyOD | **Thesis** | TU Delft MSc thesis in Applied Mathematics, Dewwret Sitaldin. Trains and deploys five PyOD detectors (OCSVM, PCA, Isolation Forest, iNNE, Feature Bagging) as the relevance-detection baseline; cites the PyOD documentation URL rather than the JMLR paper. Quote: "The PyOD package of Python contains a great amount of anomaly detection models that can be easily trained and deployed [76]." [Link](https://diamhomes.ewi.tudelft.nl/~kvuik/numanal/sitaldin_afst.pdf) |
| 356 | PyOD | **Thesis** | Florida Atlantic University PhD dissertation, Zahra Salekshahrezaee, defended Fall 2025 (posted Jan 2026). PyOD provides the Isolation Forest implementation for the noise/label-scarcity comparison study. Quote: "In this study, we make use of the IF implementation in the Python Outlier Detection (PyOD) library. PyOD is an open-source package specifically designed for detecting anomalies in various types of data [199]." [Link](https://digitalcommons.fau.edu/etd_general/223/) |
| 357 | PyOD | **Thesis** | WEAKEST OF THE CONFIRMED ROWS. ISCTE-IUL (University Institute of Lisbon) MSc dissertation in Business Intelligence Systems, Andre Gil Cardeira Martins, September 2019. PyOD is named once, in the list of Python packages the work was built on; no discussion and no citation. Real adoption but a single line of evidence. Quote: "Main Python packages that were used: - Pandas; Numpy; Keras; Tensorflow; Scikit-learn; PyOD - Outliers Detection; TS Fresh - Extract Features from Time Series; Fbprophet - Facebook Prophet; Rpy2 - Run R code in ..." [Link](http://hdl.handle.net/10071/20281) |
| 358 | PyOD | **Thesis** | Kiel doctoral thesis (Dr. rer. nat.), Andreas Lohrer, 2024, supervisor Peer Kroeger. PyOD supplies the reference implementations, and the evaluation methodology is taken from ADBench ('Inspired by ADBench ... we calculate a critical difference'), which is method adoption rather than citation. Meta-learner model selection (Zhao et al., 2021) is also discussed. Shares one embedded co-authored paper chapter with record 4 (Huenemoerder), which is why the PyOD [34] reference appears in both. Quote: "Inspired by ADBench (Han, Hu, Huang, Jiang, & Zhao, 2022), we calculate a critical difference cd based on the Wilcoxon approach with a significance level ..." [Link](https://macau.uni-kiel.de/servlets/MCRFileNodeServlet/macau_derivate_00006918//Dissertation_Andreas_Lohrer_Final.pdf) |
| 359 | PyOD | **Thesis** | National College of Ireland MSc in AI practicum, Shaik Junaid (X23354631), 2025. PyOD is one of the two libraries the whole preprocessing / training / testing pipeline runs on; no bibliography entry for PyOD. Quote: "This study employs libraries like PyOD and NetML, which are targeted at detecting anomalies in network traffic. PyOD (Python Outlier Detection) is an extensive Python library that facilitates a broad number of algorithms for outlier detection, from traditional" [Link](https://norma.ncirl.ie/9490/1/shaikjunaid.pdf) |
| 360 | PyOD | **Thesis** | Purdue University PhD dissertation, Arvind Sundaram, School of Nuclear Engineering, December 2022, DOI 10.25394/PGS.21231743.v1. PyOD supplies the Isolation Forest implementation used in the covert-cognizance anomaly experiments. Record 1 of this shard is the same dissertation at a second address. Quote: "The iForest algorithm was implemented using the PyOD library [112], a state-of-the-art anomaly detection library for time-series data." [Link](https://hammer.purdue.edu/ndownloader/files/37716714) |
| 361 | ECOD | **Thesis** | Sabanci University MSc thesis in Data Science, Yasser Zouzou, July 2024. ECOD is one of five detectors benchmarked for coordinated fake-follower detection, with per-window AUC/AP/P@50 numbers reported and a dedicated heatmap figure. Author is not a co-author of ECOD. Quote: "The Empirical-Cumulative-distribution-based Outlier Detection (ECOD) method assigns high outlier scores to data points that have a low tail probability under the joint cumulative distribution." [Link](https://research.sabanciuniv.edu/51715/1/10661021.pdf) |
| 362 | PyOD | **Thesis** | TU Wien doctoral thesis (Dr. techn.), Alexander Hartl, advisor Tanja Zseby. Real title on the cover is 'Anomaly Detection for Network Security based on Streaming Data'. PyOD is used as the runtime-performance comparison point for the author's dSalmon framework and its scikit-learn-style interface is adopted as a design decision. Quote: "PyOD is a popular Python package that provides several methods for outlier detection." [Link](https://repositum.tuwien.at/bitstream/20.500.12708/188375/1/Hartl%20Alexander%20-%202023%20-%20Anomaly%20Detection%20for%20Network%20Security%20based%20on...pdf) |
| 363 | PyOD | **Thesis** | Texas State University MSc thesis, Nader Maray, May 2023. Runs PyOD's Minimum Covariance Determinant detector and lists Pyod among the technologies; no bibliography entry for PyOD. Quote: "As for Anomaly Detection, we used 4 anomaly detection methods being: - sklearn: Variational Auto Encoder, Local Outlier Factor, Isolation Forest, One Class SVM - pyod: Minimum Covariance Determinant" [Link](https://digital.library.txst.edu/bitstreams/40623102-16fd-4cf3-a415-1d2cb0a0132f/download) |
| 364 | PyOD | **Thesis** | Umea University bachelor's thesis. PyOD builds all models (Isolation Forest, kNN, CBLOF) and the thesis devotes Section 4.2 to it; also discusses PyOD's CBLOF weighting choice. Recovered from the Wayback Machine after DiVA refused connections at the network level. Quote: "PYOD [17], a Python Toolkit for detecting outliers, is used to build the different models." [Link](https://www.diva-portal.org/smash/get/diva2:1674371/FULLTEXT01.pdf) |
| 365 | PyOD | **Thesis** | ADDRESS CORRECTED. UFU doctoral thesis (Tese, Doutorado em Ciencia da Computacao), Pedro Henrique Aparecido Damaso de Melo, 4 Oct 2022, DOI 10.14393/ufu.te.2022.587, advisors Rodrigo Sanches Miani and Pedro Frosi Rosa. Twelve one-class detectors from PyOD 1.0.4 are run across three test cases with pinned hyperparameters; COPOD is named and scored individually. The shard's handle 36068 points at an unrelated history thesis, so the recorded address is wrong even though the claim holds. Quote: "Os experimentos foram desenvolvidos usando a linguagem Python na versao 3.9 com a biblioteca PyOD 1.0.4 (ZHAO; NASRULLAH; LI, 2019)." [Link](https://repositorio.ufu.br/handle/123456789/36539) |
| 366 | PyOD | **Thesis** | UPC Barcelona (Facultat d'Informatica de Barcelona) bachelor's final project, Tomas Permanyer Rocha, 20 Jan 2026, directors Conrado Martinez Parra and Luis Antonio Belanche Munoz. The glossary states PyOD is the base for the project's algorithm implementations; ECOD and COPOD are described, benchmarked and ensembled (HBOS+ECOD ensemble, COPOD vs IForest AP comparison). Three of the lab's works cited. Quote: "PyOD Biblioteca de Python para deteccion de anomalias escalable que sirve como base para la implementacion de los algoritmos en este proyecto[3]." [Link](https://upcommons.upc.edu/handle/2117/457349) |
| 367 | PyOD | **Thesis** | UPC Barcelona doctoral thesis (Doctorat en Enginyeria Telematica), Hoang Duy Trinh, 10 June 2020, DOI 10.5821/dissertation-2117-191800, advisors Paolo Dini and Lorenza Giupponi. PyOD runs all the anomaly-detection benchmarks on 5G mobile-traffic data. Quote: "For all these above benchmarks, we used PyOD, which is a scalable implementation for detecting outlying objects in multivariate data presented in [125]." [Link](https://tdx.cat/handle/10803/669204) |
| 368 | TDC | **Thesis** | ADDRESS CORRECTED. UC Berkeley Electronic Theses and Dissertations, 'Machine Learning Methods for Protein Engineering', Hunter Morris Nisonoff, 2024, advisor Jennifer Listgarten. The permalink printed on the cover page is escholarship.org/uc/item/16r9155f. By contrast, the shard's ark 4n90r33q returns S3 AccessDenied at every content path while 16r9155f returns the PDF, so 4n90r33q is very likely a phantom. TDC is used to build the train/test splits, not merely cited. Nisonoff is not a TDC co-author (the separate 'Zhao et al., 2024' in the text is Stephen Zhao, twisted SMC). Quote: "We used the Therapeutic Data Commons (TDC) to construct 0.6/0.4 train/test splits using their default scaffold-split and using 20% of the training points for validation (Huang et al., 2021; Sorkun et al., 2019)." [Link](https://escholarship.org/uc/item/16r9155f) |
| 369 | PyOD | **Thesis** | University of Cape Town MSc Advanced Analytics dissertation, Edward Baleni, 23 July 2026, supervisors Patrick Marais and Sebnem Er. ECOD is one of five baselines run over 20 orchards with reported AUC and AP. FIVE of the lab's works appear in the bibliography: PyOD, ECOD, COPOD, ADBench (Han, Hu, Huang, Jiang, Zhao 2022) and HPOD (Zhao and Akoglu 2022). Quote: "ECOD is a state-of-the-art method used in this study, which tops a number of benchmarks in terms of speed and performance (Li et al., 2022; Han et al., 2022)." [Link](https://open.uct.ac.za/handle/11427/43712) |
| 370 | PyOD | **Thesis** | University of Florence doctoral thesis, Fahad Ahmed Khokhar, 2026, supervisors Tommaso Zoppi and Andrea Ceccarelli. All eight unsupervised classifiers in the safety-wrapper architecture come from PyOD, with COPOD and SUOD named individually and COPOD selected for the lowest-complexity monitor configuration UM7. Quote: "We select 8 algorithms belonging to different families [17] from the Python framework PyOD [16], namely: FastABOD (Angle-based), HBOS (Proximity), COPOD (Probabilistic), CBLOF (Clustering), PCA and MCD (Linear Model), IForest and SUOD (Ensembles)." [Link](https://hdl.handle.net/2158/1471584) |
| 371 | PyOD | **Thesis** | University of Padua doctoral thesis (Ingegneria dell'Informazione, XXXVIII cycle), Mattia Fanan, defended 13 Mar 2026, supervisor Ruggero Carli. Three of the four anomaly detectors in the root-cause-analysis chapter are PyOD implementations with stated hyperparameters. Four of the lab's works cited (PyOD, ECOD, XGBOD, SUOD). Quote: "The experiments were implemented in Python [194] using the PyOD [195], PySAD [196], and SHAP [180] libraries." [Link](https://hdl.handle.net/11577/3604216) |
| 372 | PyOD | **Thesis** | STRONGEST THESIS ROW IN THIS SHARD. Universitaet Bremen Dr.-Ing. dissertation, Hendrik Engbers, 10 Apr 2026, DOI 10.26092/elib/6287. All ten candidate detectors are implemented with PyOD, and the related-work chapter scores MetaOD (Zhao, Rossi, Akoglu 2020) as the only prior approach meeting five of the author's seven criteria, with LSCP assessed alongside. Three of the lab's works engaged, two of them substantively rather than as citations. Quote: "Aufgrund dieser Ueberlegungen werden die folgenden Algorithmen in die Liste der Kandidatenalgorithmen aufgenommen und mithilfe der PYOD-Bibliothek (Y. Zhao, Nasrullah und Z. Li 2019) implementiert: Clustering-Based Local Outlier Factor (CBLOF), Copula-Based Ou" [Link](https://media.suub.uni-bremen.de/handle/elib/25170) |
| 373 | PyGOD | **Thesis** | Utrecht University Applied Data Science master's thesis, Luca Lagunas, 2022. Ten PyGOD models are imported and run as the anomalous-node-detection stage of the PyGQE tool. Reference [14] is the BOND benchmark paper (Kay Liu et al., arXiv:2206.10071), so both PyGOD and BOND are named. Quote: "The library we utilized is PyGod [14], a Python library for graph outlier detection. It's developed on top of PyTorch Geometric and PyTorch and it offers several anomaly detection algorithms." [Link](https://studenttheses.uu.nl/bitstreams/ab579ac9-88c5-40a9-9212-2418cef6fa5b/download) |
| 374 | PyOD | **Thesis** | Virginia Tech MSc thesis, Nicholas Swede Merrill, June 2020. PyOD implements the OCSVM, LOF and kNN baselines; the bibliography also carries XGBOD. Quote: "One-class Support Vector Machine, Local Outlier Factor, and kth-nearest-neighbor methods are implemented using the sklearn and Pyod libraries in Python [102]." [Link](https://vtechworks.lib.vt.edu/handle/10919/98659) |
| 375 | PyOD | **Institutional** | Qualified confirmation. ETH Zurich's physics department runs a public GitLab copy of Netdata whose anomalies collector declares PyOD as its engine. The text, however, is upstream Netdata's own documentation verbatim (the file header points back at github.com/netdata/netdata). What the row records is institutional deployment of a third-party tool rather than ETH-authored coverage. The Phase A file path docs/guides/monitor/anomaly-detection-python.md does not exist in this copy. Demoted T2 to T3. Worth recording separately: 'netdata' appears zero times in the whole audit file, so the upstream Netdata PyOD integration looks like an untracked gap. Quote: "This collector uses the Python [PyOD](https://pyod.readthedocs.io/en/latest/index.html) library to perform unsupervised [anomaly detection](https://en.wikipedia.org/wiki/Anomaly_detection) on your Netdata charts and/or dimensions." [Link](https://gitlab.phys.ethz.ch/isgphys/netdata) |
| 376 | PyOD | **Institutional** | Phase A gave the bare root. Two MRCET B.Tech machine-learning lab manuals (R22 CSE and an earlier CSE edition) list PyOD on p14 among the recommended Python libraries for data cleaning. Bare list mention: no version, no link, no author name. Tier 3 course material. Neither mrcet.com nor 'Malla Reddy' appears anywhere in the audit file. Quote: "Python Libraries for Data Cleaning and Manipulation: Pandas, PyOD, NumPy, Scipy, Spacy" [Link](https://mrcet.com/pdf/Lab%20Manuals/CSE/ML%20LAB-R22.pdf) |
| 377 | Yue Zhao (person) | **Institutional** | USC ISI news roundup of USC papers at ACL 2026 names Yue Zhao on five papers with plain-language summaries. Distinct from the tracked ISI NeurIPS 2024 page (Ledger 2 #18e) and from the ISI ICML 2026 page already in news-search-candidates.jsonl line 636. Own-institution PR, tiered with the existing ISI and Viterbi university-PR rows. Quote: "Auditable Agents Yi Nian, Aojie Yuan, Haiyue Zhang, Jiate Li, Yue Zhao. As AI assistants begin sending emails, accessing files, and making decisions on their own, an important question arises: if something goes wrong, can we figure out exactly what happened?" [Link](https://www.isi.edu/news/85864/ai-to-prevent-suicide-ai-to-better-diagnose-respiratory-diseases-usc-viterbi-and-usc-stevens-at-acl-2026/) |
| 378 | PyOD, agent-audit, anywhere-agents | **Aggregator** | Cluster of 3 verified aggregator items from this round, counted as 3 items, one per link: [hvtracker.net](https://hvtracker.net/agents/pyod), [skillsllm.com](https://skillsllm.com/compare/anywhere-agents-vs-skills), [skillsllm.com](https://skillsllm.com/skill/agent-audit). |
| 379 | PyGOD, TDC | **Tutorial** | Cluster of 3 verified tutorial items from this round, counted as 3 items, one per link: [aidrugmap.com](https://www.aidrugmap.com/article/005-tdc-tutorial/), [developer.cloud.tencent.com](https://developer.cloud.tencent.com/article/1982502), [hub.baai.ac.cn](https://hub.baai.ac.cn/view/6961). |
| 380 | Aegis, CatchBench, Defenses Against Prompt Attacks Learn Surface Heuristics, ECOD, GRADE, PyOD, TDC, TrustLLM | **Community** | Cluster of 20 verified community items from this round, counted as 20 items, one per link: [api.stackexchange.com](https://api.stackexchange.com/2.3/questions/76863981?site=stackoverflow&filter=withbody), [blog.csdn.net](https://blog.csdn.net/fanfanhebaoer/article/details/121895971), [blog.csdn.net](https://blog.csdn.net/gitblog_00300/article/details/156189740), [blog.csdn.net](https://blog.csdn.net/hao_wujing/article/details/159357775), [dev.to](https://dev.to/chrisywz/open-source-ai-stacks-for-e-commerce-2025-guide-583g), [dev.to](https://dev.to/rxkov/json-csv-and-yaml-are-not-safe-formats-for-ai-agents-they-are-attack-vectors-p91), [dev.to](https://dev.to/thatechmaestro/evaluating-trustworthiness-in-distilgpt2-51bj), [developer.nvidia.com](https://developer.nvidia.com/blog/author/rohitsinghyadav/), [habr.com](https://habr.com/ru/articles/845510/), [medium.com](https://medium.com/data-science/anamoly-detection-with-pyod-fea90f0b4b42), [moltbook.com](https://moltbook.com/post/449b85ff-7146-4937-84d8-f06a45ce4971), [moltbook.com](https://moltbook.com/post/870e4c4b-de57-494b-a3a8-4d49472f0e49), [moltbook.com](https://moltbook.com/post/b13d6fc1-a792-4be3-b39e-29bb7de6316c), [news.ycombinator.com](https://news.ycombinator.com/item?id=48905248), [reddit.com](https://www.reddit.com/r/datascience/comments/1cyq5rf/anomalies_and_forecasting_with_ml/), [reddit.com](https://www.reddit.com/r/MachineLearning/comments/1afias5/d_p_multivariate_anomaly_detection_using/), [stackoverflow.com](https://stackoverflow.com/questions/56018935/python-anomaly-detection-pyod-is-not-converging), [stats.stackexchange.com](https://stats.stackexchange.com/questions/619602/why-should-i-split-the-data-when-searching-for-outliers-pyod), [tongtongsear.tistory.com](https://tongtongsear.tistory.com/986), [web.archive.org](http://web.archive.org/web/20210522173027/https://pub.towardsai.net/why-outlier-detection-is-hard-94386578be6c?gi=4821ef504fd9). |
| 381 | AD-AGENT, MemoHarness, PyOD, The Autonomy Tax | **Video** | Cluster of 5 verified video items from this round, counted as 5 items, one per link: [artificialpeerreview.com](https://www.artificialpeerreview.com/episodes/the-autonomy-tax-defense-training-breaks-llm-agents-apr-podcast-script), [youtube.com](https://www.youtube.com/watch?v=4xCHF5d4zGA), [youtube.com](https://www.youtube.com/watch?v=fjkSBdamYjo), [youtube.com](https://www.youtube.com/watch?v=HtWcLww_eJc), [youtube.com](https://www.youtube.com/watch?v=Wu6cpwJjZTc). |
| 382 | Aegis, Agent Safety Is Action Alignment, Auditable Agents, Knowledge-Extraction on RAG, PyOD | **Blog** | Cluster of 5 verified blog items from this round, counted as 5 items, one per link: [blog.theseriouscto.com](https://blog.theseriouscto.com/2026/09/supervised-ai-agents-need-contracts-not.html), [cordum.io](https://cordum.io/blog/pre-dispatch-vs-post-hoc-agent-safety), [faultlinewatch.com](https://faultlinewatch.com/the-record/fr-ai-0003/), [habr.com](https://habr.com/ru/companies/ussc/articles/1055144/), [promptfoo.dev](https://www.promptfoo.dev/lm-security-db/vuln/rag-knowledgebase-exfiltration-5cc4207b/). |
| 383 | PyOD, Revisiting Time Series Outlier Detection | **Platform** | Cluster of 4 verified platform items from this round, counted as 4 items, one per link: [docs.oracle.com](https://docs.oracle.com/en-us/iaas/tools/automlx/latest/v24.4.1/_static/notebooks/OracleAutoMLx_AnomalyDetection.html), [engineering.contentsquare.com](https://engineering.contentsquare.com/2022/datasets_and_benchmarks/), [github.com](https://github.com/Azure-Samples/Synapse/blob/main/Notebooks/PySpark/tutorial-predict-nyc-taxi-tips-onnx.ipynb), [github.com](https://github.com/ibm-cloud-architecture/vaccine-solution-main/blob/master/docs/src/pages/solution/cp4d/notebook/Saving%20Custom%20Software%20Specification%20to%20a%20Project.ipynb). |
...
**Count: 448 ecosystem adoption items across 414 rows.** Recounted 2026-09-22 from the table itself. Six cluster rows carry 40 items between them and every other row carries one, which is the whole of the gap between the two figures. The previous figure of 170 dated from 2026-08-09, when the table held 154 rows, and later rounds appended 177 more without updating it.
...
**Count: 21 first-party/community items (8 table rows plus 13 recorded in prose below).**
...
**Count: 22 awards/recognitions/encyclopedia entries (21 table rows plus the Foresight Institute grantee page, recorded in prose below).**
...
| A34 | [arXiv:2608.13558](https://arxiv.org/abs/2608.13558), "OmniScientist" (Bobo Li, Hao Fei, Tianjie Ju, Mong Li Lee, Wynne Hsu; NUS and Oxford) | The Autonomy Tax | Introduction, page 1, reference `bib.bib41`. Cited as foundation for agent tool use alongside ReAct, Toolformer and Reflexion. **Falsifies the last of the four cap-artifact zeros**, and the first external citer this work has. | 2026-09-13 |
| A35 | [arXiv:2609.12582](https://arxiv.org/abs/2609.12582) | Auditable Agents | NovaFabric (Mohsen Seyedkazemi Ardebili, NovaFabric Project, ORCID 0000-0002-1166-6559, single independent author) engages Auditable Agents substantively, summarizing its mechanism in the paper's own prior-art paragraph. Demoted T3->T5 (arXiv preprint). Strongest arXiv-tier engagement with the Auditable Agents line in this shard. | 2026-09-22 |
| A36 | [arXiv:2609.20519](https://arxiv.org/abs/2609.20519) | MemoHarness | SoL-Pi, an NVIDIA + NTU + MIT paper (Haozhe Liu, Tian Ye, Sensen Gao, Qihang Cao, Yitong Li, Mingchen Zhuge, Duomin Wang, Ruihua Zhang, Ping Luo, Jiawang Bian, Lei Zhu, Ligeng Zhu, Enze Xie, Song Han, none on the MemoHarness byline), describes MemoHarness's mechanism in prose. Substantive, and the strongest institutional affiliation among the arXiv rows in this shard. Demoted T3->T5 on surface; the NVIDIA/MIT affiliation is the reason a maintainer may want to look at it anyway. | 2026-09-22 |
| A37 | AgentDock / SteerBench Team [Link](https://steerbench.com/work/paper) | The Autonomy Tax (Li & Zhao) | Substantive related-work engagement rather than a bare citation: SteerBench-Work (Oguz Serdar, Cuneyt Mertayak, AgentDock, arXiv:2608.12654) calls The Autonomy Tax 'the closest prior observation' to its own finding. Independent authors. The separate 'Jiale Zhao' in the acknowledgments is a different person, as Phase A flagged. Phase A T3 -> T5. | 2026-09-22 |

**Count: 34 confirmed external academic citations of the agent-auditing line, across 37 rows.** Three rows carry the `/pdf/` URL of a paper another row holds at `/abs/`, so this table is counted by distinct identifier rather than by row. Recounted 2026-09-22. The previous figure of 11 dated from 2026-08-09, and later rounds appended 23 more rows without updating it.
...
| S49 | ACM | XGBOD: Improving Supervised Outlier Detection with Unsupervised Representation Learning | ACM SIGKDD 2023, 'Deep Weakly-supervised Anomaly Detection' (Guansong Pang, Singapore Management University; Chunhua Shen, Zhejiang; Huidong Jin, CSIRO Data61; Anton van den Hengel, Adelaide). They ran XGBOD's official implementation as a comparison method and published its numbers in two appendix tables, and acknowledge the person who ran it - adoption, not a bibliography entry. Held at the Phase A T1 ceiling. COUNTER-ARGUMENT FOR THE SKEPTIC: the 2026-09-12 pass placed a comparable ACM SIGKDD 2026 record (MacrOData, cites and benchmarks against ADBench) at T2. The audit also has an open rule gap on where 'ran the cited benchmark and published its numbers' belongs. Four independent institutions; no co-author overlap with Zhao and Hryniewicki. [Link](https://dl.acm.org/doi/10.1145/3580305.3599302) |
| S50 | ACM | XGBOD | Strongest form of use in this shard: the whole paper is an extension of XGBOD and names the PI in its abstract and subtitle. Phase A asserted an XGBOD title the Crossref deposit does not carry; the abstract supplies it instead. OpenAlex drops this citation entirely, so identifier-graph sweeps cannot find it. [Link](https://doi.org/10.1145/3459955.3460595) |
| S51 | American Chemical Society | TDC | J. Chem. Inf. Model. 2026, 66(14):8045-8058. TDC is the object of the whole audit, the benchmark protocol is followed operationally, and tdcommons.ai is cited as the instruction source. Self-citation screen: the 10 authors are at Receptor.AI, Taras Shevchenko Kyiv National University and IOCB Prague, none on the TDC author list. THREE TDC-line entries, not one, and [24] and [25] both carry Yue Zhao in the author block. The stance is critical of leaderboard reproducibility. Distinct from the Chemistry World article the 2026-09-12 pass refuted, which audits 51 configurations across four sources. TDC keeps the standing coauthorship-dilution annotation. [Link](https://doi.org/10.1021/acs.jcim.6c00819) |
| S52 | CEUR-WS | SUOD: Toward Scalable Unsupervised Outlier Detection | CEUR Workshop Proceedings Vol-4146, 'Meta-ensemble methods for outlier detection in real estate' (Khomyshyn, Pastukh, Yatsyshyn, Baran, Ushenko; Ternopil and Chernivtsi). The most substantive PyOD-family use in this shard: LSCP and SUOD are the actual aggregation engines of every meta-ensemble the paper builds, and four FORTIS works are cited (combo, PyOD, LSCP, SUOD). Workshop proceedings, so capped at T3. Authors independent. [Link](https://ceur-ws.org/Vol-4146/paper04.pdf) |
| S53 | CEUR-WS | LSCP: Locally Selective Combination in Parallel Outlier Ensembles | CEUR Workshop Proceedings Vol-4057 (CITI 2025, Ternopil), 'Comparative analysis of ensemble methods for outlier detection in real estate' (Khomyshyn, Pastukh, Yatsyshyn, Zagorodna, Baran; Ternopil Ivan Puluj National Technical University). Substantive: LSCP and DCSO are described at length and PyOD supplies every algorithm evaluated. Workshop proceedings, so capped at T3. Authors independent. [Link](https://ceur-ws.org/Vol-4057/paper5.pdf) |
| S54 | Cell Press | TDC | Patterns (Cell Press) 2024, by Fan, Yu, Zhang, Chen, Sun, Zhang, Chen, Xiao, Wu, Li, Zheng, Luo and Wang (Shanghai Institute of Materia Medica, independent). It devotes a Methods subsection to TDC, uses its splits on five ADMET datasets and compares against the TDC baseline AttFp-TDC. Substantive. Held at T2. Note a first PMC lookup returned the wrong article (PMC11289513); the DOI search gave the right one. [Link](https://doi.org/10.1016/j.patter.2024.100991) |
| S55 | Elsevier | PyOD | Expert Systems with Applications 2025 (Dabouei, Parayil Shibu, Dalal, Cao, MacWilliams, Kangas, Xu, Carnegie Mellon, independent) runs PyOD's ECOD implementation as an experimental baseline on laboratory video. ECOD confirmed in the publisher-deposited reference list; the PyOD sentence is the Phase A quote, not independently re-read. Held at T2. [Link](https://doi.org/10.1016/j.eswa.2025.126581) |
| S56 | Elsevier | TODS | Expert Systems with Applications 2024 evaluation survey (Mejri, Lopez-Fuentes, Roy, Chernakov, Ghorbel, Aouada, Univ. of Luxembourg SnT, independent) builds its anomaly-type analysis on the Lai/Zhao TODS taxonomy; ADBench also cited at b21. Substantive, not bibliographic. Demoted T1->T2 (specialist journal). [Link](https://doi.org/10.1016/j.eswa.2024.124922) |
| S57 | Frontiers | ADBench | Frontiers in Public Health: AMAnD, a biosurveillance metagenome anomaly detector, is built on PyOD's autoencoder and DeepSVDD and cites ADBench as the benchmark that justifies the choice. PyOD is named in the body but has no reference entry, the weaker Ledger 7 form (cf. row S6). Phase A recorded ADBench alone and missed the PyOD use. [Link](https://doi.org/10.3389/fpubh.2023.1181911) |
| S58 | Frontiers | PyOD | Frontiers in Robotics and AI: every multivariate baseline in the GVFOD comparison comes from PyOD, and PyOD's default hyperparameters are used deliberately as the no-fault-data deployment condition. Substantive, not bibliographic. [Link](https://doi.org/10.3389/frobt.2024.1214043) |
| S59 | Frontiers | ECOD, ADBench | Frontiers in Artificial Intelligence: ADBench drives the baseline selection and ECOD is one of four baselines carrying numbers in Table 2. Reference number corrected: Phase A gave ECOD as [18], the printed number is [19]. [Link](https://doi.org/10.3389/frai.2024.1441205) |
| S60 | ICLR 2026 | JailDAM | ICLR 2026 paper 'From Sure to Sorry: Detecting Jailbreak in Large Vision Language Model via JailNeurons' (Yuyou Gan, Qingming Li, Junhao Li, Zhi Chen, Jinbao Li, Xiaoming Li, Shouling Ji; Zhejiang University et al.). It runs JailDAM as one of seven detection baselines and publishes its numbers. Substantive use, not a bibliography entry. Authors independent of the JailDAM co-author list. [Link](https://proceedings.iclr.cc/paper_files/paper/2026/file/096347b4efc264ae7f07742fea34af1f-Paper-Conference.pdf) |
| S61 | IEEE | COPOD | IEEE ICISS 2023 (ICT for Smart Society) runs COPOD as the outlier-removal step for lithology classification, and COPOD is in the title. Reference number corrected from [2] to [12]. [Link](https://doi.org/10.1109/iciss59129.2023.10291626) |
| S62 | IEEE / arXiv | BOND: Benchmarking Unsupervised Outlier Node Detection on Static Attributed Graphs | ICDE 2023 paper 'Unsupervised Graph Outlier Detection: Problem Revisit, New Insight, and Superior Method' (Yihong Huang, Liping Wang, ECNU; Fan Zhang, GZHU; Xuemin Lin, SJTU) adopts BOND's injection protocol as the standard and cites two FORTIS works. METHOD NOTE: the canonical pdf_term_scan.py returned zero here because the PDF renders the title as 'Bond:' and the scanner's word-boundary rule for short uppercase tool names is case-sensitive; a case-insensitive raw grep found it. Authors independent. [Link](https://arxiv.org/abs/2210.12941) |
| S63 | MDPI | COPOD | TITLE CORRECTION: 'Distribution-Aware Outlier Detection in High Dimensions: A Scalable Parametric Approach' (Phase A said 'A Survey'). COPOD is discussed twice in the body and listed in the abbreviations table; PyOD detectors are run inside the published Monte Carlo script although PyOD has no reference entry. The MDPI 403 yields to the mdpi-res.com CDN path, which is worth adding to the route list. [Link](https://doi.org/10.3390/math14010077) |
| S64 | MDPI | PyOD, ECOD, COPOD, ADBench | Deepest PyOD adoption in this shard: a 30-method benchmark for inborn errors of metabolism in which most detectors come from PyOD, each with reproducible settings, and the repository URL github.com/yzhao062/Pyod appears in the appendix. Four works cited (PyOD, COPOD, ECOD, ADBench); Phase A recorded ADBench's reference number only. [Link](https://doi.org/10.3390/metabo13010097) |
| S65 | MDPI | PyOD | Strongest reproducibility form in this shard: pinned PyOD 2.0.5, the PI named by full name in the body, and the canonical docs URL. TWO works, not one: LSCP is run as one of seven evaluated estimators with numbers, and Phase A recorded only PyOD. Sensors 2026, urban-intersection trajectory anomaly detection. [Link](https://doi.org/10.3390/s26103027) |
| S66 | MDPI | PyGOD | Sensors (MDPI) 2026 ANIMATE (Hu, Zhang, Zhu, Hou, NUDT, independent) cites BOND at ref 22 inside a grouped in-text list, and CONAD at ref 12. No standalone substantive use of BOND. Held at T2; MDPI direct routes 403, PMC mirror used. [Link](https://doi.org/10.3390/s26103176) |
| S67 | National Academy of Sciences | TDC | PNAS 2023 ConPLex (Rohit Singh, Samuel Sledzieski, Bryan Bryson, Lenore Cowen, Bonnie Berger, MIT / Ragon / Tufts, independent) evaluates on the TDC-DG benchmark and reports itself top of the TDC leaderboard. The strongest substantive TDC use in this shard. Held at T1. [Link](https://doi.org/10.1073/pnas.2220778120) |
| S68 | Nature Publishing Group | TDC | Nature Communications 2023 KPGT (Han Li, Ruotian Zhang, Yaosen Min, Dacheng Ma, Dan Zhao, Jianyang Zeng, Tsinghua, independent) benchmarks on TDC ADMET tasks and cites both the TDC NeurIPS paper and the Nature Chemical Biology paper. Substantive benchmark use. Held at T1. [Link](https://doi.org/10.1038/s41467-023-43214-1) |
| S69 | Nature Publishing Group | ADBench | Nature Communications 2024 (Gao, Shen, Wang, Chan, Hon, Li, Nanjing Univ. of Aeronautics and Astronautics + Hong Kong Observatory, independent). ADBench dictates the baseline selection and XGBOD is run as a comparison model. Two works, one document. Held at T1. [Link](https://doi.org/10.1038/s41467-024-51597-y) |
| S70 | Nokia Solutions and Networks | PyOD | Substantive in-body use: Nokia Solutions and Networks engineers (Wroclaw) with Wroclaw UST and BTU Cottbus run PyOD's ECOD/IFOREST/LOF/MCD/OCSVM as the benchmark baselines for a 4G/5G/6G base-station testbed detector. Enterprise adoption published at an academic preprint outlet. Phase A T2 -> T3. [Link](https://arxiv.org/abs/2608.08373) |
| S71 | PMLR | Automatic Unsupervised Outlier Model Selection | STRONGEST RECORD IN THE SHARD. AutoML Conference 2023 (PMLR v224), Jose Manuel Navarro, Alexis Huet and Dario Rossi, all Huawei Technologies. This is adoption rather than citation. MetaOD's 200-feature meta-feature set is used directly as the paper's feature basis (M_MetaOD), and MetaOD is the named state-of-the-art comparator in the headline results. Its license is credited in an appendix, and three separate Yue Zhao works are cited (MetaOD, ELECT, the internal-strategies study). PyOD is named as the LOF implementation source. ADDRESS CORRECTION: the Phase A URL (v222/navarro24a) does not exist; the real address is v224/navarro23a. TIER NOTE: this is an upgrade over the Phase A T2 ceiling, made deliberately on the strength of the evidence: a major corporate research lab adopting the method, which the Tier 1 rubric covers as a major research institution. Dario Rossi is not Ryan A. Rossi; the authors are independent. [Link](https://proceedings.mlr.press/v224/navarro23a.html) |
| S72 | SoLAR | SynC: A Unified Framework for Generating Synthetic Population with Gaussian Copula | SoLAR Learning Letters Vol. 1 (2023 inaugural issue), 'Synthetic data generator for student data serving learning analytics' (Chen Zhan, Oscar Blessed Deho, Xuwei Zhang, Srecko Joksimovic, Maarten de Laat; University of South Australia). Substantive: the paper attributes the Synthetic Data Vault GaussianCopula generator to the SynC paper and then benchmarks it against CTGAN, CopulaGAN and TVAE on real student data. Crossref deposits zero references for this DOI, so the PDF was required. New low-volume journal, so capped at T3. Authors independent. [Link](https://doi.org/10.59453/khzw9006) |
| S73 | Springer Nature | PyOD, ECOD, COPOD | TITLE CORRECTION: 'STAR_outliers: a python package that separates univariate outliers from non-normal distributions' (Phase A said 'into contextual and non-contextual'). Substantive head-to-head use: PyOD's detectors are the benchmark STAR_outliers is measured against, and the ECOD and COPOD scoring functions are reproduced in the text. Critical in stance, real in use. [Link](https://doi.org/10.1186/s13040-023-00342-0) |
| S74 | Springer Nature | PyOD | Data Mining and Knowledge Discovery (Springer) 2026, Zhong Li / Jiayang Shi / Matthijs van Leeuwen (Leiden University, independent) run PCA, OCSVM, iForest and HBOS from PyOD as comparison baselines for GNN-based log anomaly detection. Substantive in-body use. Held at T2. [Link](https://doi.org/10.1007/s10618-026-01235-6) |
| S75 | University of Oxford | PyOD | NOT a thesis: this is the accepted manuscript of a peer-reviewed journal article by Brian B. Sheil and Stephen K. Suryasentana (University of Oxford) and Wen-Chieh Cheng (Xi'an University of Architecture and Technology), hosted on ORA. PyOD implements the paper's anomaly detectors. Belongs in Ledger 7 (peer-reviewed, substantive in-body use), not the thesis pool. The shard's ORA UUID does not exist. [Link](https://ora.ox.ac.uk/objects/uuid:36458ded-dbc1-49b1-8ebd-34429566b742) |
| S76 | VLDB Endowment | The Need for Unsupervised Outlier Model Selection: A Review and Evaluation of Internal Evaluation Strategies | PVLDB 17(12) 2024 tutorial paper (Qinghua Liu OSU, Boniol Inria, Palpanas Paris Cité, Paparrizos OSU, independent) cites Ma, Zhao, Zhang & Akoglu at ref [29] and uses it in-body as the internal-evaluation reference. Demoted T1->T2 (specialist DB venue). [Link](https://doi.org/10.14778/3685800.3685842) |
| S77 | arXiv | SocialMaze: A Benchmark for Evaluating Social Reasoning in Large Language Models | arXiv preprint 'COBRA-Skills' (Pingchen Lu, Xiangyi Wang, Xiang Li, Jie Mao, Zikun Qu, Junfeng Luo, Yao Shu, Bryan Kian Hsiang Low, Zhongxiang Dai; CUHK-Shenzhen, Tianjin, HKUST-GZ, NUS). It uses SocialMaze as one of six evaluation benchmarks and follows its official soft scoring. Substantive benchmark use in a preprint. Authors independent. [Link](https://arxiv.org/abs/2609.11682) |
| S78 | arXiv (the Phase A venue claim of ICML 2026 is unverified) | Secure On-Device Video OOD Detection Without Backpropagation | arXiv preprint in ICML position-paper format, 'Position: Weight Space Should Be a First-Class Generative AI Modality' (Zhangyang Wang, Peihao Wang, UT Austin; Kai Wang, NUS). Names SecDOOD as one of the systems demonstrating hypernetwork-based on-device learning, with the full ten-author reference. Substantive in-body naming. Phase A called it 'ICML 2026'; the PDF carries no venue line, so the venue claim is unverified and the record is scored as a preprint. [Link](https://arxiv.org/abs/2605.18632) |

**Count: 78 peer-reviewed articles with substantive in-body use, one per row.** Recounted 2026-09-22 from the table itself. The previous figure of 15 dated from 2026-08-09, and later rounds appended 63 more rows without updating it.
...
*Rebuilt 2026-09-22. Every entry in the previous list was an April-to-July 2026 event that had already
passed, so the section had stopped being a forward list. Each entry below links the page it was
confirmed on, fetched 2026-09-22. A date the page did not show is marked as unconfirmed rather than
supplied from memory.*

### Open Consultations and Comment Periods

| Closes | Body | What it covers | What the lab would submit |
|---|---|---|---|
| **2026-09-23** | [Canada ISED, "Advancing AI Transparency"](https://ised-isde.canada.ca/site/ised/en/have-your-say-advancing-ai-transparency-canada) | One pillar is tracking AI-agent activity and interactions | Implicit Execution Tracing (arXiv:2603.17445) and Auditable Agents (arXiv:2604.05485) |
| **2026-10-06** | [NIST AI 200-2 ipd, the TEVV-Athlon framework](https://www.nist.gov/artificial-intelligence/ai-research/tevv-athlon-framework-evaluating-ai-systems) | AI testing, evaluation, verification and validation | CatchBench, AD-AGENT, TrustLLM and Auditable Agents as TEVV instruments. The page gives the address as TEVV-Athlon at nist.gov, with "NIST AI 200-2" in the subject line. As published, the draft is a verified negative on every FORTIS term. |
| Unconfirmed | [China TC260, 智能体系统开发安全指南 exposure draft](https://www.tc260.org.cn/front/bzzqyjDetail.html?id=20260918151240) | Agent over-privilege, tool-call security, memory pollution | The FORTIS over-privilege benchmark (arXiv:2605.09163), Aegis (arXiv:2603.12621) and agent-audit (arXiv:2603.22853). TC260's notice list confirms the draft was issued for public comment on 2026-09-18. Its detail page loads by script and the closing date could not be read on two routes. |
| Rolling | [UK AISI Inspect Evals](https://github.com/UKGovernmentBEIS/inspect_evals) | Community evaluation register, submitted by pull request | CatchBench, WeClawArena, DoxBench and the FORTIS benchmark. Still the one standing open window. |
| Active | [IETF SCITT, `draft-emirdag-scitt-ai-agent-execution`](https://datatracker.ietf.org/doc/draft-emirdag-scitt-ai-agent-execution/) | An AI agent execution profile for supply-chain transparency | Reasoning hashes and cryptographic execution receipts from Implicit Execution Tracing and the `auditable` SDK |

### Conference Roles the Audit Had Not Listed

These are the PI's own service roles, recorded on the CV and confirmed on each venue's organizer
page. They are listed because each is a visibility surface, not because a round discovered them.

1. **IEEE BigData 2026**: Future Faculty Highlights Co-Chair. [Track submissions close 2026-10-24](https://bigdataieee.org/BigData2026/calls/future-faculty-highlights/).
2. **WSDM 2027**: Publicity Co-Chair. [Organizers](https://www.wsdm-conference.org/2027/organizers.html).
3. **AAAI 2027**: Doctoral Consortium Co-Chair. [Organizers](https://aaai.org/conference/aaai/aaai-27/organizers/).
4. **NeurIPS 2026**: Area Chair, per the CV.

### Outlets That Have Covered This Work and Would Plausibly Cover It Again

Each is a counted Ledger 2 row. On TrustLLM: SC Media and SC World (May 2026), Transformer (alignment
coverage), and RBC Borealis (as the enterprise trustworthiness benchmark). MarkTechPost covered
DrugAgent. USC ISI news and CMU CSD news are institutional but editorial. EurekAlert via AAAS carried
TDC through the Insilico Medicine release, and The Agent Times covers the agent-auditability line.
...
| Ledger | Rows in table | Counted items | Why the two differ |
|--------|---------------|---------------|--------------------|
| Government/policy citations (Ledger 1) | 38 | **38** | One document per row. Row 21 (Japan MIC) added 2026-09-22. |
| Government technical reports (Ledger 1b) | 13 | **9** | The table's stated basis is DOE national-laboratory reports with substantive use, which is rows G1 through G8 and G13. Rows G9 through G12, added 2026-08-13, sit outside that basis: two are DOE-hosted documents rather than laboratory reports, and two (arxiv.org, pubs.rsc.org) are not government documents at all. They are left in place and left uncounted pending re-placement. |
| External third-party media (Ledger 2) | 122 | **121** | Row 22 (SitePoint) is present but marked REMOVED and is not counted. |
| Ecosystem adoption (Ledger 3) | 414 | **448** | Six cluster rows carry 40 items between them, following the folding convention used on 2026-06-02. Every other row is one item. |
| First-party/community (Ledger 4) | 8 | **21** | 8 table rows plus 13 items recorded in prose beneath the table. |
| Awards/recognitions (Ledger 5) | 21 | **22** | 21 table rows plus the Foresight grantee page recorded in prose. |
| Agent-auditing academic citations (Ledger 6) | 37 | **34** | Three rows carry the `/pdf/` URL of a paper another row holds at `/abs/`. Count by distinct identifier, not by row. |
| Scientific uptake (Ledger 7) | 78 | **78** | One peer-reviewed article per row. |
| **Total** | **731** | **771** | |

**The previous total of 582 was wrong, and so was the arithmetic that produced it.** The eight
per-ledger figures printed above it added to 364, not 582, so the total was never reproducible from
the table it claimed to sum. Separately, each per-ledger figure had stopped tracking its own table.
Ledger 3 published 170 against 331 rows, Ledger 2 published 80 against 108, Ledger 7 published 15
against 48, and Ledger 6 published 11 against 34. Rounds had been appending rows without updating
the count line beneath the table, and the Summary copied the stale count lines rather than the
tables. Both faults are fixed by the same change: the figures above are counted from the live
tables, and the count line under each table now carries the same number.

The recount is not growth. Of the 771 counted items, 165 were added by this round's verification
(Ledger 1 +1, Ledger 2 +14, Ledger 3 +117, Ledger 6 +3, Ledger 7 +30). One more, Ledger 1b row G13,
is a 2026-09-12 promotion whose row was never written. The pre-round tables held 605 items against
a published 582, so 23 items were on the tables and outside the total before this round began.

**The rule that prevents recurrence** is the one already stated at the end of Ledger 1, extended by
one clause. Write the ledger row in the same commit that records the promotion, and update that
table's count line in the same commit as the row. `scripts/ci_check_site.py` gates the patent figure
across four surfaces; it does not yet gate the ledger counts against their tables, which is the
obvious next check to add.
...
- **94 patents** citing PyOD/COPOD/ECOD/LSCP/SUOD/XGBOD/TODS/ADBench/PyGOD/CONAD/LUNAR/MetaOD, counted as distinct inventions rather than publication numbers (China 70, US 14, WIPO 3, EPO 3, Germany 1, Japan 1, Korea 1, Slovakia 1). Ledger 3 carries 102 patent rows. Eight are a second or third publication of an invention another row holds, which leaves 94. The eight pairs, and the family check that cleared all 13 of this round's additions, are in "## 2026-09-22 Pass".
